...
public class LogInfo {
    public static void track(final String format, final Object... args) {
        track(String.format(format, args), false);
    }

    public static void track(final Object o) {
        track(o, false);
    }

    public synchronized static void track_methods(final Object o, final String[] methodNames) throws Exception {
     ...
     * @throws Exception
     */
    public synchronized static void track_method(final Object o, final String methodName, final Object... args)
            throws Exception {
        ...
        if (targetMethod == null) {
            final String msg = String.format("Couldn't find method %d in class %s", methodName, c.getName());
    ...
    public synchronized static void track_method(final Object o, final String methodName) throws Exception {
        track_method(o, methodName, new Object[0]);
    }

    public synchronized static void track(final Object o, final boolean printAllLines) {
        track(o, printAllLines, false);
    }

    public synchronized static void track(final Object o, final boolean printAllChildLines,
            final boolean printIfParentPrinted) {
        if (indWithin()) {
    ...
    public static <T> T end_track(final T x) {
        end_track();
        return x;
    }
    ...
    public synchronized static void end_track() {
        ...
        if (indWithin()) {
            if (thisRun().newLine()) { // Note that we pay for the line only at
                ...
                indLevel++;
                final int n = thisRun().numOmitted();
                if (n > 0)
                ...
                // Print time
                final StopWatch ct = childRun().watch;
                if (ct.ms > 1000) {
                    rawPrint(" [" + ct);
                    if (indLevel > 0) {
                        final StopWatch tt = thisRun().watch;
    ...
    // Normal printing
    public static void logs(final String format, final Object... args) {
        logs(String.format(format, args));
    }

    public synchronized static void logs(final Object o) {
        if (forcePrint || (indWithin() && thisRun().newLine()))
            printLines(o);
    }

    // Always print
    public synchronized static void logsForce(final String format, final Object... args) {
        printLines(String.format(format, args));
    }

    public synchronized static void logsForce(final Object o) {
        thisRun().newLine();
        printLines(o);
    }

    // Print if parent printed
    public static void logss(final String format, final Object... args) {
        logss(String.format(format, args));
    }

    public synchronized static void logss(final Object o) {
        if (parentPrinted())
            thisRun().forcePrint();
        logs(o);
    }
    ...
    @Deprecated
    public static void dbg(final String format, final Object... args) {
        dbg(String.format(format, args));
    }

    public static void dbgs(final String format, final Object... args) {
        dbg(String.format(format, args));
    }

    public static void dbg(final Object o) {
        logss("DBG: " + o);
    }

    public static void rants(final String format, final Object... args) {
        rant(String.format(format, args));
    }

    public static void rant(final Object o) {
        logss("RANT: " + o);
    }
    ...
    public static void errors(final String format, final Object... args) {
        error(String.format(format, args));
    }
    ...
    public static void warnings(final String format, final Object... args) {
        warning(String.format(format, args));
    }
    ...
    public static void fails(final String format, final Object... args) {
        fail(String.format(format, args));
    }

    public static void fail(final Object o) {
    ...
    public static <T> void printList(final String s, final List<T> items) {
        track(s, true);
        for (final T x : items)
            logs(x);
        end_track();
    ...
    private static void rawPrint(final Object o) {
        if (out != null) {
            out.print(o);
            out.flush();
        }
...
class LogRun {
    ...
    boolean newLine() {
        ...
        final int msPerLine = LogInfo.msPerLine;
        if (numLines <= 2 || // Print first few lines anyway.
            ...
            final long elapsed_ms = watch.getCurrTimeLong();
