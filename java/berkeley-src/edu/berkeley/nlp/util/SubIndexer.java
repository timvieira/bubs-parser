...
/**
 * Maintains a two-way map between a set of (objects, substate) pairs and contiguous integers from 0 to the number of
 * objects. It also maintains a map from (object, substate) pairs to contiguous integers. Use get(i) to look up object
 * i, and indexOf(object) to look up the index of an object. This class was modified to accomodate objects that can have
 * several substates. It is assumed that if an object is added several times, it will be with the same number of
 * substates.
 * 
 * @author Dan Klein
