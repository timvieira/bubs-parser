...
public class TDoubleMap<T> extends AbstractTMap<T> implements Iterable<TDoubleMap<T>.Entry>, Serializable {
    ...
    /*
     * public void clear() { // Keep the same capacity num = 0; for(int i = 0; i < keys.length; i++) keys[i] = null; }
     */
    public void gut() {
        values = null;
    } // Save memory
    ...
    private int find(final T key, final boolean modify) {
        ...
        if (mapType == MapType.SORTED_LIST) {
            ...
            if (!locked && modify && (num > loadFactor * capacity || capacity <= num + 1)) {
                /*
                 * if(locked) throw new RuntimeException("Cannot make new entry for " + key +
