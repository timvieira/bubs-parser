...
/**
 * A base class for Head Finders similar to the one described in Michael Collins' 1999 thesis. For a given constituent
 * we perform
 * <p/>
 * for categoryList in categoryLists for index = 1 to n [or n to 1 if R->L] for category in categoryList if category
 * equals daughter[index] choose it.
 * <p/>
 * with a final default that goes with the direction (L->R or R->L) For most constituents, there will be only one
 * category in the list, the exception being, in Collins' original version, NP.
 * <p/>
 * It is up to the overriding base class to initialize the map from constituent type to categoryLists,
 * "nonTerminalInfo", in its constructor. Entries are presumed to be of type String[][]. Each String[] is a list of
 * categories, except for the first entry, which specifies direction of traversal and must be one of "right", "left" or
 * "rightdis" or "leftdis".
 * <p/>
 * "left" means search left-to-right by category and then by position "leftdis" means search left-to-right by position
 * and then by category "right" means search right-to-left by category and then by position "rightdis" means search
 * right-to-left by position and then by category
 * <p/>
 * <p/>
 * 10/28/2002 -- Category label identity checking now uses the equals() method instead of ==, so not interning category
 * labels shouldn't break things anymore. (Roger Levy) <br>
 * 2003/02/10 -- Changed to use TreebankLanguagePack and to cut on characters that set off annotations, so this should
 * work even if functional tags are still on nodes. <br>
 * 2004/03/30 -- Made abstract base class and subclasses for CollinsHeadFinder, ModCollinsHeadFinder,
 * SemanticHeadFinder, ChineseHeadFinder (and trees.icegb.ICEGBHeadFinder, trees.international.negra.NegraHeadFinder,
...
public abstract class AbstractCollinsHeadFinder implements HeadFinder, Serializable {
     ...
     * 
     * @param t The parse tree to examine the daughters of. If this is a leaf, <code>null</code> is returned
     * @return The daughter parse tree that is the head of <code>t</code>
     * @see Tree#percolateHeads(HeadFinder) for a routine to call this and spread heads throughout a tree
     */
    public Tree<String> determineHead(Tree<String> t) {
        if (nonTerminalInfo == null) {
            throw new RuntimeException("Classes derived from AbstractCollinsHeadFinder must"
    ...
    /*
     * Called by determineHead and may be overridden in subclasses if special treatment is necessary for particular
     * categories
     */
    ...
    /**
     * Attempt to locate head daughter tree from among daughters. Go through daughterTrees looking for things from a set
     * found by looking up the motherkey specifier in a hash map, and if you do not find one, take leftmost or rightmost
     * thing iff deflt is true, otherwise return <code>null</code>.
     */
    protected Tree<String> traverseLocate(List<Tree<String>> daughterTrees, String[] how, boolean deflt) {
        ...
        if (how[0].equals("left")) {
            ...
            twoloop: for (headIdx = daughterTrees.size() - 1; headIdx >= 0; headIdx--) {
                ...
                for (int i = 1; i < how.length; i++) {
                    if (DEBUG) {
                        System.err.println("Testing for whether " + how[i] + " == " + childCat + ": "
                                + ((how[i].equals(childCat)) ? "true" : "false"));
                    }
                    if (how[i].equals(childCat)) {
                        found = true;
                        break twoloop;
