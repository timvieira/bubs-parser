...
/**
 * Implements the HeadFinder found in Michael Collins' 1999 thesis. Except: we've added a head rule for NX, which
 * returns the leftmost item. No rule for the head of NX is found in any of the versions of Collins' head table that we
 * have (did he perhaps use the NP rules for NX? -- no Bikel, CL, 2005 says it defaults to leftmost). These rules are
 * suitable for the Penn Treebank.
 * <p>
 * May 2004: Added support for AUX and AUXG to the VP rules; these cause no interference in Penn Treebank parsing, but
 * means that these rules also work for the BLLIP corpus (or Charniak parser output in general). Feb 2005: Fixes to
 * coordination reheading so that punctuation cannot become head.
 * 
...
public class CollinsHeadFinder extends AbstractCollinsHeadFinder {
    ...
    protected int postOperationFix(int headIdx, List<Tree<String>> daughterTrees) {
        if (headIdx >= 2) {
            ...
            if (prevLab.equals("CC")) {
                ...
                while (newHeadIdx >= 0 && t.isPreTerminal() && tlp.isPunctuationTag(t.getLabel())) {
                    newHeadIdx--;
                }
                if (newHeadIdx >= 0) {
                    headIdx = newHeadIdx;
    ...
    public CollinsHeadFinder(TreebankLanguagePack tlp) {
        ...
        nonTerminalInfo.put("ADVP", new String[][] { { "right", "RB", "RBR", "RBS", "FW", "ADVP", "TO", "CD", "JJR",
                "JJ", "IN", "NP", "JJS", "NN" } });
        ...
        nonTerminalInfo.put("NAC", new String[][] { { "left", "NN", "NNS", "NNP", "NNPS", "NP", "NAC", "EX", "$", "CD",
        ...
        nonTerminalInfo.put("QP", new String[][] { { "left", "$", "IN", "NNS", "NN", "JJ", "RB", "DT", "CD", "NCD",
                "QP", "JJR", "JJS" } });
        ...
        nonTerminalInfo.put("SBAR", new String[][] { { "left", "WHNP", "WHPP", "WHADVP", "WHADJP", "IN", "DT", "S",
                "SQ", "SINV", "SBAR", "FRAG" } });
        nonTerminalInfo.put("SBARQ", new String[][] { { "left", "SQ", "S", "SINV", "SBARQ", "FRAG" } });
        nonTerminalInfo.put("SINV", new String[][] { { "left", "VBZ", "VBD", "VBP", "VB", "MD", "VP", "S", "SINV",
                "ADJP", "NP" } });
        ...
        nonTerminalInfo.put("NP", new String[][] { { "rightdis", "NN", "NNP", "NNPS", "NNS", "NX", "POS", "JJR" },
                { "left", "NP" }, { "rightdis", "$", "ADJP", "PRN" }, { "right", "CD" },
    ...
    /**
     * Go through trees and determine their heads and print them. Just for debuggin'. <br>
     * Usage: <code>
