...
public final class SloppyMath {
    ...
    public static double lambert(double v, double u) {
        ...
        /*
         * //Use asymptotic expansion w = log(z) - log(log(z)) for most z double summand = (z==0) ? 1 : 0; double tmp =
         * Math.log(z+summand);// + i*b*6.28318530717958648; double w = tmp - Math.log(tmp + summand);
         * 
         * //For b = 0, use a series expansion when close to the branch point //k = find(b == 0 & abs(z +
         * 0.3678794411714423216) <= 1.5); tmp = Math.sqrt(5.43656365691809047*z + 2) - 1;// + i*b*6.28318530717958648;
         * //w(k) = tmp(k); w = tmp;
         * 
         * for (int k=1; k<36; k++){ // Converge with Halley's iterations, about 5 iterations satisfies //the tolerance
         * for most z double c1 = Math.exp(w); double c2 = w*c1 - z; summand = (w != -1) ? 1 : 0; double w1 = w +
         * summand; double dw = c2/(c1*w1 - ((w + 2)*c2/(2*w1))); w = w - dw;
         * 
    ...
    public static double oneTailedFishersExact(int k, int n, int r, int m) {
        if (k < 0 || k < (m + r) - n || k > r || k > m || r > n || m > n) {
            throw new IllegalArgumentException("Invalid Fisher's exact: " + "k=" + k + " n=" + n + " r=" + r + " m="
                    + m + " k<0=" + (k < 0) + " k<(m+r)-n=" + (k < (m + r) - n) + " k>r=" + (k > r) + " k>m=" + (k > m)
                    + " r>n=" + (r > n) + "m>n=" + (m > n));
        }
        // exploit symmetry of problem
        if (m > n / 2) {
