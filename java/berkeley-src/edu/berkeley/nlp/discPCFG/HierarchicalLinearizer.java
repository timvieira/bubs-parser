...
/**
 * similar to cascading linearizer but doesnt compute the grammars explicitly instead uses hierarchical rules and merges
 * back unused splits
 * 
 * @author petrov
 * 
    ...
    public void increment(double[] counts, UnaryRule rule, double[] weights, boolean isGold) {
        ...
        for (int cp = 0; cp < nSubstates; cp++) {
            ...
            for (int np = 0; np < nSubstates; np++) {
                ...
                if (val > 0) {
                    if (isGold)
                        counts[thisStartIndex + unaryMapping[finalLevel][cp][np]] += val;
                    else
                        counts[thisStartIndex + unaryMapping[finalLevel][cp][np]] -= val;
                    weights[curInd] = 0;
                }
        ...
        for (int lp = 0; lp < nSubstates; lp++) {
            for (int rp = 0; rp < nSubstates; rp++) {
                ...
                for (int np = 0; np < nSubstates; np++) {
                    ...
                    if (val > 0) {
                        if (isGold)
                            counts[thisStartIndex + binaryMapping[finalLevel][lp][rp][np]] += val;
                        else
                            counts[thisStartIndex + binaryMapping[finalLevel][lp][rp][np]] -= val;
                        weights[curInd] = 0;
                    }
