...
public class ParsingObjectiveFunction implements ObjectiveFunction {
    ...
    private void ensureCache(double[] proposed_x) {
        if (requiresUpdate(proposed_x)) {
            ...
            if (objective < bestObjectiveSoFar && !ConditionalTrainer.Options.dontSaveGrammarsAfterEachIteration) {
                ...
                ParserData pData = new ParserData(lexicon, grammar, spanPredictor, Numberer.getNumberers(),
    ...
    class Calculator implements Callable {
        ...
        Calculator(StateSetTreeList myT, String consN, int i, Grammar gr, Lexicon lex, SpanPredictor sp, int dimension,
                boolean notProject) {
        ...
        /**
         * The most important part of the classifier learning process! This method determines, for the given weight
         * vector x, what the (negative) log conditional likelihood of the data is, as well as the derivatives of that
            ...
            for (Tree<StateSet> stateSetTree : myTrees) {
                ...
                if (consName != null) {
                    ...
                    if (cons.length != yield.size()) {
                        System.out.println("My ID: " + myID + ", block: " + block + ", sentence: " + i);
                        System.out.println("Sentence length (" + yield.size() + ") and constraints length ("
                                + cons.length + ") do not match!");
                        System.exit(-1);
                    }
                }
                ...
                double goldLL = (ConditionalTrainer.Options.hierarchicalChart) ? eParser.doInsideOutsideScores(
                        stateSetTree, noSmoothing, debugOutput, eParser.spanScores) : gParser.doInsideOutsideScores(
                        stateSetTree, noSmoothing, debugOutput, eParser.spanScores);
                ...
                if (false) { // compute exhaustive iS/oS to get exact
                    ...
                    double exactLL = eParser.doConstrainedInsideOutsideScores(yield, null, noSmoothing, null, null,
                            false);
    ...
    public ParsingObjectiveFunction(Linearizer linearizer, StateSetTreeList trainTrees, double sigma,
            int regularization, String consName, int nProc, String outName, boolean doNotProjectConstraints,
            boolean combinedLexicon) {
        this.sigma = sigma;
    ...
    protected Calculator newCalculator(boolean doNotProjectConstraints, int i) {
        return new Calculator(trainingTrees[i], consBaseName, i, grammar, lexicon, spanPredictor, dimension,
     ...
     * (non-Javadoc)
     * 
     * @see edu.berkeley.nlp.classify.ObjectiveFunction#getLogProbabilities(edu.berkeley .nlp.classify.EncodedDatum,
     * double[], edu.berkeley.nlp.classify.Encoding, edu.berkeley.nlp.classify.IndexLinearizer)
     */
    public <F, L> double[] getLogProbabilities(EncodedDatum datum, double[] weights, Encoding<F, L> encoding,
