...
public class ConstrainedArrayParser extends ArrayParser implements Callable {
    ...
    public Tree<String> getBestConstrainedParse(final List<String> sentence, final List<String> posTags) {// List<Integer>[][]
         ...
         * 
         * List<Integer> possibleParentSt = possibleStates[12][13]; for (int pState : possibleParentSt){
         * System.out.println(pState + " " + (String) tagNumberer.object(pState) + " iScore "+
        ...
        for (int diff = length; diff >= 1; diff--) {
            for (int start = 0; start + diff <= length; start++) {
                ...
                for (final int pState : possibleParentSt) {
                    // this check should be unnecessary. if we get a null pointer exception here, then we did not
                    // initialize the arrays properly - slav
                ...
                /*
                 * for (int rState = 0; rState < numStates; rState++) { int max1 = narrowLExtent[end][rState]; if (max1
                 * < start) { continue; } BinaryRule[] rules = grammar.splitRulesWithRC(rState); for (int r = 0; r <
                 * rules.length; r++) { BinaryRule br = rules[r];
                 * 
                 * if (oScore[start][end][br.parentState]==null) {continue;} int lState = br.leftChildState; int min1 =
                 * narrowRExtent[start][lState]; if (max1 < min1) { continue; } int min = min1; int max = max1; if (max
                 * - min > 2) { int min2 = wideLExtent[end][rState]; min = (min1 > min2 ? min1 : min2); if (max1 < min)
                 * { continue; } int max2 = wideRExtent[start][lState]; max = (max1 < max2 ? max1 : max2); if (max <
                 * min) { continue; } }
                 * 
                 * double[][][] scores = br.getScores(); for (int split = min; split <= max; split++) { if
                 * (oScore[start][split][lState] == null) continue; if (oScore[split][end][rState] == null) continue;
                 * for (int lp=0; lp<scores[0].length; lp++){ double lS = iScore[start][split][lState][lp]; if (lS ==
                 * Double.NEGATIVE_INFINITY) { continue; } for (int rp=0; rp<scores[0][0].length; rp++){ double rS =
                 * iScore[split][end][rState][rp]; if (rS == Double.NEGATIVE_INFINITY) { continue; } for (int np=0;
                 * np<scores.length; np++){ double oS = oScore[start][end][br.parentState][np]; if (oS ==
                 * Double.NEGATIVE_INFINITY) { continue; } double pS = scores[np][lp][rp];
                 * 
                 * double totL = pS + rS + oS; if (totL > oScore[start][split][lState][lp]) {
                 * System.err.println("Shouldn't occur!"); System.exit(1); oScore[start][split][lState][lp] = totL; }
                 * double totR = pS + lS + oS; if (totR > oScore[split][end][rState][rp]) {
                 * System.err.println("Shouldn't occur!"); System.exit(1); oScore[split][end][rState][rp] = totR; } } }
                 * } } } }
                 */
            }
        }
