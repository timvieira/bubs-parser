...
public class SophisticatedLexicon implements java.io.Serializable, Lexicon {
    ...
    /**
     * If logarithmMode is true, then all scores are returned as log probabilities. Otherwise, they are returned as
     * probabilities.
     */
    ...
    public String toString() {
        ...
        // word counter (c_W)
        /*
         * sb.append("WORD-COUNTER (c_W):\n"); PriorityQueue<String> pq = wordCounter.asPriorityQueue(); while
         * (pq.hasNext()) { int priority = (int)Math.round(pq.getPriority()); String element = pq.next();
         * sb.append(element +" "+priority+"\n"); }
         * 
         * sb.append("--------------------------------------------------\n"); sb.append("TAG-COUNTER (c_T):\n"); for
         * (int state=0; state<tagCounter.length; state++){ String tagState = (String)n.object(state); for (int
         * substate=0; substate<tagCounter[state].length; substate++){ double prob = tagCounter[state][substate]; if
         * (prob==0) continue; sb.append(tagState +"_"+substate +" "+prob+"\n"); } }
         * 
         * sb.append("--------------------------------------------------\n"); sb.append("UNSEEN-TAG-COUNTER (c_T):\n");
         * for (int state=0; state<unseenTagCounter.length; state++){ String tagState = (String)n.object(state); for
         * (int substate=0; substate<unseenTagCounter[state].length; substate++){ double prob =
         * unseenTagCounter[state][substate]; if (prob==0) continue; sb.append(tagState +"_"+substate +" "+prob+"\n"); }
         * }
         * 
         * sb.append("--------------------------------------------------\n");
    ...
    public SophisticatedLexicon(final short[] numSubStates, final int smoothingCutoff, final double[] smoothParam,
     ...
     */
    @SuppressWarnings("unchecked")
    public SophisticatedLexicon splitAllStates(final int[] counts, final boolean moreSubstatesThanCounts, final int mode) {
        ...
        final SophisticatedLexicon lexicon = new SophisticatedLexicon(newNumSubStates, this.smoothingCutoff, smooth,
    ...
    public double[] score(final String word, final short tag, final int loc, final boolean noSmoothing,
        ...
        /*
         * double power = 1.0; // raise to the power for (int i=0; i<resultArray.length; i++) { resultArray[i] =
    ...
    public void trainTree(final Tree<StateSet> trainTree, final double randomness, final Lexicon oldLexicon,
            final boolean secondHalf, final boolean noSmoothing, final int threshold) {
        // scan data
    ...
    public void removeUnlikelyTags(double threshold, final double exponent) {
    ...
    public SophisticatedLexicon projectLexicon(final double[] condProbs, final int[][] mapping,
            final int[][] toSubstateMapping) {
        final short[] newNumSubStates = new short[numSubStates.length];
        for (int state = 0; state < numSubStates.length; state++) {
            newNumSubStates[state] = (short) toSubstateMapping[state][0];
        }
        final Smoother newSmoother = this.smoother.copy();
        newSmoother.updateWeights(toSubstateMapping);
        final SophisticatedLexicon newLexicon = new SophisticatedLexicon(newNumSubStates, this.smoothingCutoff,
                this.smooth, newSmoother, this.threshold);

        final double[][] newTagCounter = new double[newNumSubStates.length][];
        final double[][] newUnseenTagCounter = new double[newNumSubStates.length][];
        if (!isConditional) {
            for (int tag = 0; tag < numSubStates.length; tag++) {
                // update tag counters
                ...
                if (unseenWordToTagCounters[tag] != null) {
                    ...
                    for (final String word : unseenWordToTagCounters[tag].keySet()) {
                        final double[] scores = unseenWordToTagCounters[tag].get(word);
                        final double[] newScores = new double[newNumSubStates[tag]];
                        for (int i = 0; i < numSubStates[tag]; i++) {
                            newScores[toSubstateMapping[tag][i + 1]] += condProbs[mapping[tag][i]] * scores[i];
                        }
            ...
            final double[][][] newCondWeights = new double[conditionalWeights.length][conditionalWeights[0].length][];
    ...
    public SophisticatedLexicon copyLexicon() {
        ...
        final SophisticatedLexicon newLexicon = new SophisticatedLexicon(newNumSubStates, this.smoothingCutoff,
                this.smooth, this.smoother, this.threshold);

        final double[][] newTagCounter = ArrayUtil.copy(tagCounter);
        final double[][] newUnseenTagCounter = ArrayUtil.copy(unseenTagCounter);
        for (int tag = 0; tag < numSubStates.length; tag++) {
            if (wordToTagCounters[tag] != null) {
                newLexicon.wordToTagCounters[tag] = new HashMap<String, double[]>();
                for (final String word : wordToTagCounters[tag].keySet()) {
                    final double[] scores = wordToTagCounters[tag].get(word);
                    final double[] newScores = scores.clone();
                    newLexicon.wordToTagCounters[tag].put(word, newScores);
                }
            }
            // update wordToTagCounters
    ...
    public void delinearizeLexicon(final double[] probs) {
        int ind = 0;
        // Numberer wordNumberer = Numberer.getGlobalNumberer("words");
        for (int tag = 0; tag < wordToTagCounters.length; tag++) {
            if (wordToTagCounters[tag] != null) {
                for (final String word : wordToTagCounters[tag].keySet()) {
                    final double[] scores = new double[numSubStates[tag]];
                    for (int i = 0; i < scores.length; i++) {
    ...
    public void setConditional(final boolean b) {
        this.isConditional = b;
    }

    public double[] scoreConditional(final String word, final short tag, final int loc, final boolean noSmoothing,
            final boolean isSignature) {
        if (isSignature)
    ...
    public double[] getConditionalSignatureScore(final String sig, final short tag, final boolean noSmoothing) {
        final double[] resultArray = new double[numSubStates[tag]];
     ...
     */
    public int getLinearIndex(final String word, final int tag) {
        // TODO Auto-generated method stub
        return 0;
    }
    ...
    @SuppressWarnings("unchecked")
    public SophisticatedLexicon remapStates(final Numberer thisNumberer, final Numberer newNumberer) {
        final SophisticatedLexicon remappedLexicon = copyLexicon();
        ...
        for (short s = 0; s < newNumberer.size(); s++) {
            final short translatedState = translateState(s, newNumberer, thisNumberer);
