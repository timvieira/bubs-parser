...
/**
 * Simple default implementation of a lexicon, which scores word, tag pairs with P(word|tag)
 * 
 * instead of dealing with words as strings we will map them to integers with a wordIndexer. to further simplify things
 * each tag will have its own mapping from wordIndex to a tag-specific index so that we don't have to deal with
 * unobserved events
 * 
...
public class SimpleLexicon implements java.io.Serializable, Lexicon {
    ...
    public SimpleLexicon copyLexicon() {
        ...
        /*
         * if (linearIndex!=null) copy.linearIndex = ArrayUtil.clone(linearIndex); if (toBeIgnored!=null)
         * copy.toBeIgnored = toBeIgnored.clone();
         */
        return copy;
    }
     ...
     * 
     * @param randomness (currently ignored)
     * @param mode 0 or 1: previous value plus noise 2: just noise (for log-linear grammars with cascading
     *            regularization)
     * @return
     */
    @SuppressWarnings("unchecked")
