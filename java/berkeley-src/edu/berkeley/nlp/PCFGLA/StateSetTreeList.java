...
public class StateSetTreeList extends AbstractCollection<Tree<StateSet>> {
     ...
     * @return
     */
    public static Tree<StateSet> stringTreeToStatesetTree(Tree<String> tree, short[] numStates,
            boolean allSplitTheSame, Numberer tagNumberer) {
        Tree<StateSet> result = stringTreeToStatesetTree(tree, numStates, allSplitTheSame, tagNumberer, false, 0, tree
                .getYield().size());
    ...
    private static Tree<StateSet> stringTreeToStatesetTree(Tree<String> tree, short[] numStates,
            boolean allSplitTheSame, Numberer tagNumberer, boolean splitRoot, int from, int to) {
        if (tree.isLeaf()) {
        ...
        for (Tree<String> child : tree.getChildren()) {
            ...
            Tree<StateSet> newChild = stringTreeToStatesetTree(child, numStates, allSplitTheSame, tagNumberer, true,
                    from, from + length);
            from += length;
