...
public class SpanPredictor implements Serializable {
    ...
    public double[] scoreSpan(int previousIndex, int firstIndex, int lastIndex, int followingIndex) {
        ...
        for (int c = 0; c < nClasses; c++) {
            // if (c==1) continue;
            if (useFirstAndLast)
                result[c] *= firstWordScore[firstIndex][c] * lastWordScore[lastIndex][c];
            if (usePreviousAndNext) {
                if (previousIndex >= 0)
    ...
    public double[] countGoldSpanFeatures(StateSetTreeList trainTrees) {
        ...
        for (Tree<StateSet> tree : trainTrees) {
            ...
            countGoldSpanFeaturesHelper(tree, words, firstWordCount, lastWordCount, previousWordCount, nextWordCount,
                    beginPairsCount, endPairsCount, punctuationCount, punctuationSig);
        }
    ...
    private void countGoldSpanFeaturesHelper(Tree<StateSet> tree, List<StateSet> words, int[][] firstWordCount,
            int[][] lastWordCount, int[][] previousWordCount, int[][] nextWordCount, int[][] beginPairsCount,
            int[][] endPairsCount, int[][] punctuationCount, int[][] punctuationSignatures) {
    ...
    public int[][] getPunctuationSignatures(List<StateSet> sentence, boolean update, Counter<String> punctSigCounter) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            masked[i] = (thisStateSet.wordIndex > 0 && isPunctuation[thisStateSet.wordIndex]) ? thisStateSet.getWord()
                    : X;
        }
