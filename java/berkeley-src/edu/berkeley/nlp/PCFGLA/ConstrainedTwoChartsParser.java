...
public class ConstrainedTwoChartsParser extends ConstrainedArrayParser {
            ...
            for (int start = 0; start < (length - diff + 1); start++) {
                        ...
                        for (int split = min; split <= max; split++) {
                                            ...
                                            for (int nnp = 0; nnp < divisor; nnp++) {
                                                ...
                                                if (viterbi) {
                                                    unscaledScoresToAdd[p] = Math
                                                            .max(unscaledScoresToAdd[p], thisRound);
                                                } else {
                            ...
                            if (parentScale != currentScale) {
                                if (parentScale == Integer.MIN_VALUE) { // first
                                    ...
                                    ScalingTools.scaleArrayToScale(iScorePreU[start][end][pState], parentScale,
                                            newScale);
                            ...
                            for (int np = 0; np < nParentStates; np++) {
                                if (viterbi) {
                                    iScorePreU[start][end][pState][np] = Math.max(iScorePreU[start][end][pState][np],
                ...
                for (int pState = 0; pState < numSubStatesArray.length; pState++) {
                    ...
                    for (int r = 0; r < unaries.length; r++) {
                        ...
                        for (int np = 0; np < nParentStates; np++) {
                            if (viterbi) {
                                iScorePostU[start][end][pState][np] = Math.max(iScorePostU[start][end][pState][np],
                                        unscaledScoresToAdd[np]);
                            } else {
                        ...
                        if (val > 0) {
                            if (viterbi) {
                                iScorePostU[start][end][pState][np] = Math
                                        .max(iScorePostU[start][end][pState][np], val);
                            } else {
            ...
            for (int start = 0; start + diff <= length; start++) {
                ...
                for (int cState = 0; cState < numSubStatesArray.length; cState++) {
                    ...
                    for (int r = 0; r < rules.length; r++) {
                        ...
                        for (int cp = 0; cp < nChildStates; cp++) {
                            if (viterbi) {
                                oScorePostU[start][end][cState][cp] = Math.max(oScorePostU[start][end][cState][cp],
                        ...
                        if (val > 0) {
                            if (viterbi) {
                                oScorePostU[start][end][cState][cp] = Math
                                        .max(oScorePostU[start][end][cState][cp], val);
                            } else {
                            ...
                            if (DoubleArrays.max(scoresToAdd) != 0) {// oScale[start][end][pState]!=Integer.MIN_VALUE
                                ...
                                if (leftScale != currentScale) {
                                    if (leftScale == Integer.MIN_VALUE) { // first
                                        ...
                                        ScalingTools.scaleArrayToScale(oScorePreU[start][split][lState], leftScale,
                                                newScale);
                                ...
                                for (int cp = 0; cp < nLeftChildStates; cp++) {
                                    if (scoresToAdd[cp] > initVal) {
                                        if (viterbi) {
                                            oScorePreU[start][split][lState][cp] = Math.max(
                            ...
                            if (DoubleArrays.max(unscaledScoresToAdd) != 0) {// oScale[start][end][pState]!=Integer.MIN_VALUE
                                ...
                                if (rightScale != currentScale) {
                                    if (rightScale == Integer.MIN_VALUE) { // first
                                        ...
                                        ScalingTools.scaleArrayToScale(unscaledScoresToAdd, currentScale, newScale);
                                        ScalingTools.scaleArrayToScale(oScorePreU[split][end][rState], rightScale,
                                                newScale);
                                ...
                                for (int cp = 0; cp < nRightChildStates; cp++) {
                                    if (unscaledScoresToAdd[cp] > initVal) {
                                        if (viterbi) {
                                            oScorePreU[split][end][rState][cp] = Math.max(
    ...
    void initializeChart(List<StateSet> sentence, boolean noSmoothing, List<String> posTags) {
        ...
        for (StateSet word : sentence) {
            ...
            for (short tag = 0; tag < numSubStatesArray.length; tag++) {
                ...
                /*
                 * if (start==1){ System.out.println(word+" +TAG "+(String)tagNumberer
    ...
    public Tree<String> getBestConstrainedParse(List<String> sentence, List<String> posTags,
        ...
        double ll = doConstrainedInsideOutsideScores(testSentenceStateSet, allowedStates, noSmoothing, null, posTags,
                viterbi);
    ...
    /**
     * Assumes that inside and outside scores (sum version, not viterbi) have been computed. In particular, the
     * narrowRExtent and other arrays need not be updated.
     */
            ...
            for (int start = 0; start < (length - diff + 1); start++) {
                    ...
                    for (int pState = 0; pState < numSubStatesArray.length; pState++) {
                        ...
                        for (int i = 0; i < parentRules.length; i++) {
                            ...
                            for (int split = min; split <= max; split++) {
                                ...
                                if (allowedSubStates[split][end][rState] == null)
                                    continue;
                                double scalingFactor = ScalingTools.calcScaleFactor(oScale[start][end][pState]
                                        + iScale[start][split][lState] + iScale[split][end][rState] - tree_scale);
                                if (scalingFactor == 0)
                                    continue;
                ...
                for (int pState = 0; pState < numSubStatesArray.length; pState++) {
                    ...
                    for (int r = 0; r < unaries.length; r++) {
                        ...
                        double ruleScore = 0;
                        double scalingFactor = ScalingTools.calcScaleFactor(oScale[start][end][pState]
                                + iScale[start][end][cState] - tree_scale);
                        if (scalingFactor == 0) {
                            continue;
                        }
            ...
            for (int end = start + 1; end <= length; end++) {
                ...
                for (int state = 0; state < numStates; state++) {
                    if (allowedSubStates2 == null) { // then we parse without
                        ...
                        if (allSubstates)
                            Arrays.fill(allowedSubStates[start][end][state], true);
                        else {
        ...
        for (int start = 0; start < length; start++) {
            ...
            for (int tag = 0; tag < lastState; tag++) {
                ...
                if (allowedSubStates[start][start + 1][tag] == null)
                    continue;
                double scalingFactor = ScalingTools.calcScaleFactor(oScale[start][start + 1][tag]
                ...
                linearizer.increment(probs, currentStateSet, tag, tmpCountsArray, false); // probs[startIndexWord+substate]
                                                                                          // += weight;
     ...
     * @return
     */
    public boolean[][][][] getPossibleStates(List<String> testSentence, Tree<StateSet> tree, double threshold,
        ...
        sb.append(", from: " + previouslyPossibleSub + " (" + previouslyPossible + ") to: " + possibleSubStates + " ("
            ...
            for (int end = start + 1; end <= length; end++) {
                ...
                for (int state = 0; state < lastState; state++) {
                    ...
                    for (int substate = 0; substate < numSubStatesArray[state]; substate++) {
                        ...
                        double scalingFactor = ScalingTools.calcScaleFactor(oScale[start][end][state]
                                + iScale[start][end][state] - tree_scale);
                        if (scalingFactor == 0)
                            continue;

                        double tmp = Math.max(iS * oScorePreU[start][end][state][substate],
    ...
    /**
     * Returns the best parse, the one with maximum expected labelled recall. Assumes that the maxc* arrays have been
     * filled.
     */
    ...
    public Tree<String> extractBestMaxRuleParse2(int start, int end, int state, List<String> sentence) {
        ...
        if (posLevel) {
            ...
            if (split == -1) {
                ...
                System.err.println("The score is " + maxcScore[start][end][state] + " and the state is supposed to be "
                        + stateStr);
                System.err.println("The insideScores are " + Arrays.toString(iScorePostU[start][end][state])
                        + " and the outsideScores are " + Arrays.toString(oScorePostU[start][end][state]));
                ...
                for (short start2 = 0; start2 < length; start2++) {
                    for (short tag = 0; tag < numSubStatesArray.length; tag++) {
                        ...
                        if (maxcScore[start2][start2 + 1][tag] > 0)
                            System.err.println("The maxcScore for word " + start2 + " is "
    ...
    public void checkScores(Tree<StateSet> tree) {
        ...
        for (int substate = 0; substate < numSubStatesArray[state]; substate++) {
            if ((node.getIScale() == iScale[from][to][state])
                    && (!SloppyMath.isGreater(iScorePostU[from][to][state][substate], node.getIScore(substate)))) {
                if (!allowedSubStates[from][to][state][substate])
                    System.out.println("This state was pruned!");
                else {
                    System.out
                            .println("Gold iScore is higher for state " + state + " from " + from + " to " + to + "!");
                    System.out.println("Gold " + node.getIScore(substate) + " all "
            ...
            if ((node.getOScale() == oScale[from][to][state]) && (!SloppyMath.isGreater(tmpB, tmpA))) {
                if (!allowedSubStates[from][to][state][substate])
                    ...
                    System.out
                            .println("Gold oScore is higher for state " + state + " from " + from + " to " + to + "!");
                    System.out.println("Gold " + node.getOScore(substate) + " all "
                            + oScorePostU[from][to][state][substate]);
                }
            }
        }
        for (Tree<StateSet> child : tree.getChildren()) {
    ...
    public void tallyConditionalLoss(Tree<StateSet> tree, double[][][] deltas, double[][] mergeWeights) {
        ...
        for (int i = 0; i < numSubStatesArray[state]; i = i + 2) {
            ...
            if (Double.isNaN(deltas[state][i][j])) {
                ...
                System.out.println(Arrays.toString(tmp1) + " " + Arrays.toString(tmp2) + " " + combinedGoldScore + " "
    ...
    public Tree<String> extractBestViterbiParse(int gState, int gp, int start, int end, List<String> sentence,
            ...
            for (int i = 0; i < parentRules.length; i++) {
                ...
                for (int lp = 0; lp < scores.length; lp++) {
                    for (int rp = 0; rp < scores[lp].length; rp++) {
                        if (scores[lp][rp] == null)
                            continue;
                        double score = ScalingTools.scaleToScale(scores[lp][rp][gp]
                                * iScorePostU[start][split][lState][lp] * iScorePostU[split][end][rState][rp],
                                iScale[start][split][lState] + iScale[split][end][rState], iScale[start][end][gState]);
        ...
        for (int r = 0; r < unaries.length; r++) {
            ...
            for (int cp = 0; cp < scores.length; cp++) {
                if (scores[cp] == null)
                    continue;
                double score = ScalingTools.scaleToScale(scores[cp][gp] * iScorePreU[start][end][cState][cp],
