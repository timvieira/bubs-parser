...
public class CorpusStatistics {
    ...
    /*
     * Counts how many different 'things' (non-terminals or terminals for the POS) appear under a given nonterminal
     * symbol. Currently POS and other nonterminals are handled the same way. We might to change that.
     */
    ...
    private void addParent(Tree<StateSet> tree) {
        ...
        if (!tree.isPreTerminal()) {
            if (tree.getChildren().size() == 1) {
                UnaryRule r = new UnaryRule(parentState, tree.getChildren().get(0).getLabel().getState(),
                        new double[1][1]);
                unaryRuleCounter.incrementCount(r, 1.0);
            } else {
                BinaryRule r = new BinaryRule(parentState, tree.getChildren().get(0).getLabel().getState(), tree
                        .getChildren().get(1).getLabel().getState(), new double[1][1][1]);
