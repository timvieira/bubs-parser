...
public class HierarchicalBinaryRule extends BinaryRule {
    ...
    /**
     * before: scores[leftSubState][rightSubState][parentSubState] gives score for this rule now: have a hierarchy of
     * refinements
     */
            ...
            for (int lChild = 0; lChild < newLStates; lChild++) {
                for (int rChild = 0; rChild < newRStates; rChild++) {
                    for (int parent = 0; parent < newPStates; parent++) {
                        this.scores[lChild][rChild][parent] += scoresThisLevel[lChild / divisorL][rChild / divisorR][parent
                                / divisorP];
                    }
                }
            }
    ...
    @Override
    public HierarchicalBinaryRule splitRule(short[] numSubStates, short[] newNumSubStates, Random random,
