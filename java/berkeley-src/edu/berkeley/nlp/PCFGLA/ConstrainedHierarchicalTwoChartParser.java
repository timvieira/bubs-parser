...
public class ConstrainedHierarchicalTwoChartParser extends ConstrainedTwoChartsParser {
            ...
            for (int start = 0; start < (length - diff + 1); start++) {
                        ...
                        for (int split = min; split <= max; split++) {
                            ...
                            if (parentScale != currentScale) {
                                if (parentScale == Integer.MIN_VALUE) { // first
                                    ...
                                    ScalingTools.scaleArrayToScale(h_iScorePreU[start][end][pState][finalLevel],
                            ...
                            for (int np = 0; np < nParentStates; np++) {
                                if (viterbi) {
                                    h_iScorePreU[start][end][pState][finalLevel][np] = Math.max(
                ...
                for (int pState = 0; pState < numSubStatesArray.length; pState++) {
                    ...
                    for (int r = 0; r < unaries.length; r++) {
                        ...
                        if (parentScale != currentScale) {
                            if (parentScale == Integer.MIN_VALUE) { // first
                                ...
                                int newScale = Math.max(currentScale, parentScale);
                                ScalingTools.scaleArrayToScale(unscaledScoresToAdd, currentScale, newScale);
                                ScalingTools.scaleArrayToScale(h_iScorePostU[start][end][pState][finalLevel],
                                        parentScale, newScale);
                                parentScale = newScale;
                            }
                        }
                        for (int np = 0; np < nParentStates; np++) {
                            if (viterbi) {
                                h_iScorePostU[start][end][pState][finalLevel][np] = Math.max(
                    ...
                    if (somethingChanged) {
                        ...
                        ScalingTools.scaleArrayToScale(h_iScorePostU[start][end][pState][finalLevel], parentScale,
                                newScale);
                        iScale[start][end][pState] = newScale;
    ...
    private final boolean computeInsideScore(int start, int split, int end, HierarchicalAdaptiveBinaryRule rule,
            boolean viterbi) {
            ...
            for (int start = 0; start + diff <= length; start++) {
                ...
                for (int cState = 0; cState < numSubStatesArray.length; cState++) {
                    ...
                    for (int r = 0; r < rules.length; r++) {
                        ...
                        if (childScale != currentScale) {
                            if (childScale == Integer.MIN_VALUE) { // first time
                                ...
                                ScalingTools.scaleArrayToScale(unscaledScoresToAdd, currentScale, newScale);
                                ScalingTools.scaleArrayToScale(h_oScorePostU[start][end][cState][finalLevel],
                                        childScale, newScale);
                                childScale = newScale;
                            }
                        }
                        for (int cp = 0; cp < nChildStates; cp++) {
                            if (viterbi) {
                                h_oScorePostU[start][end][cState][finalLevel][cp] = Math.max(
                    ...
                    if (somethingChanged) {
                        ...
                        ScalingTools.scaleArrayToScale(h_oScorePreU[start][end][cState][finalLevel],
                                scaleBeforeUnaries, newScale);
                        ScalingTools.scaleArrayToScale(h_oScorePostU[start][end][cState][finalLevel], childScale,
                                newScale);
                            ...
                            if (DoubleArrays.max(scoresToAdd) != 0) {// oScale[start][end][pState]!=Integer.MIN_VALUE
                                ...
                                if (leftScale != currentScale) {
                                    if (leftScale == Integer.MIN_VALUE) { // first
                                        ...
                                        ScalingTools.scaleArrayToScale(h_oScorePreU[start][split][lState][finalLevel],
                                                leftScale, newScale);
                            ...
                            if (DoubleArrays.max(unscaledScoresToAdd) != 0) {// oScale[start][end][pState]!=Integer.MIN_VALUE
                                ...
                                if (rightScale != currentScale) {
                                    if (rightScale == Integer.MIN_VALUE) { // first
                                        ...
                                        ScalingTools.scaleArrayToScale(unscaledScoresToAdd, currentScale, newScale);
                                        ScalingTools.scaleArrayToScale(h_oScorePreU[split][end][rState][finalLevel],
                                                rightScale, newScale);
                                ...
                                for (int cp = 0; cp < nRightChildStates; cp++) {
                                    if (unscaledScoresToAdd[cp] > initVal) {
                                        if (viterbi) {
                                            h_oScorePreU[split][end][rState][finalLevel][cp] = Math.max(
    ...
    private final boolean computeOutsideScore(int start, int split, int end, HierarchicalAdaptiveBinaryRule rule,
            boolean viterbi) {
        int pState = rule.parentState;
        int lState = rule.leftChildState;
        int rState = rule.rightChildState;
    ...
    private final double computeRuleScore(int start, int end, HierarchicalAdaptiveUnaryRule rule, double tree_score,
            ...
            for (int start = 0; start < (length - diff + 1); start++) {
                ...
                for (short pState = 0; pState < lastStateU; pState++) {
                    ...
                    for (UnaryRule ur : unaries) {
                        ...
                        boolean foundSomething = computeExpectedCount(start, end, (HierarchicalAdaptiveUnaryRule) ur,
            ...
            for (int end = start + 1; end <= length; end++) {
                ...
                for (int state = 0; state < lastState; state++) {
                    ...
                    for (int substate = 0; substate < numSubStatesArray[state]; substate++) {
                        ...
                        double tmp = Math.max(iS * h_oScorePreU[start][end][state][finalLevel][substate],
                                h_iScorePreU[start][end][state][finalLevel][substate] * oS);
    ...
    void doInsideScores(Tree<StateSet> tree, boolean noSmoothing, boolean debugOutput, double[][][] spanScores) {
        ...
        if (tree.isPreTerminal()) {
            ...
            case 1:
                ...
                HierarchicalAdaptiveUnaryRule urule = (HierarchicalAdaptiveUnaryRule) grammar.getUnaryRule(pState,
                        cState);
            ...
            case 2:
                ...
                HierarchicalAdaptiveBinaryRule brule = (HierarchicalAdaptiveBinaryRule) grammar.getBinaryRule(pState,
                        lState, rState);
    ...
    @Override
    public double doInsideOutsideScores(Tree<StateSet> tree, boolean noSmoothing, boolean debugOutput,
            double[][][] spanScores) {
    ...
    public void incrementExpectedGoldCounts(Linearizer linearizer, double[] probs, Tree<StateSet> tree,
        ...
        case 1:
            ...
            HierarchicalAdaptiveUnaryRule urule = (HierarchicalAdaptiveUnaryRule) grammar.getUnaryRule(parentState,
                    childState);
        ...
        case 2:
            ...
            HierarchicalAdaptiveBinaryRule brule = (HierarchicalAdaptiveBinaryRule) grammar.getBinaryRule(parentState,
                    lChildState, rChildState);
            ...
            for (int end = start + 1; end <= length; end++) {
                ...
                for (int state = 0; state < lastState; state++) {
                    ...
                    for (int substate = 0; substate < numSubStatesArray[state]; substate++) {
                        ...
                        if (posterior > 1.01) {
                            System.out.println("too much posterior s:" + start + " e:" + end + " state " + state + " "
                ...
                if (result[start][end][0] > 2.01) {
                    System.out.println("too much in the sum, start " + start + " end " + end + "  "
                            + result[start][end]);
