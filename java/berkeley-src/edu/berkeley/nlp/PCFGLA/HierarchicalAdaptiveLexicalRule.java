...
public class HierarchicalAdaptiveLexicalRule implements Serializable {
    ...
    private Pair<Short, Integer> updateMapping(short myID, int nextSubstate, int myDepth, int finalDepth,
            Tree<Double> tree) {
        if (tree.isLeaf()) {
    ...
    private int fillScores(double previousScore, int nextSubstate, int myDepth, int finalDepth, Tree<Double> tree,
            final boolean usingOnlyLastLevel) {
        if (tree.isLeaf()) {
            double myScore = (usingOnlyLastLevel) ? Math.exp(tree.getLabel()) : Math.exp(previousScore
                    + tree.getLabel());
            if (myDepth == finalDepth) {
