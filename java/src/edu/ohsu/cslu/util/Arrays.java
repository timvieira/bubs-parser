...
public class Arrays {
    ...
    /**
     * Reverse-sort parallel float[] and short[] arrays based on the values in <code>x</code> (identical to
     * {@link Arrays#sort(float[], short[])} except for the comparison)
     */
    public static void reverseSort(final float[] x, final short[] p) {
        reverseSort1(x, p, 0, x.length);
    }

    private static void reverseSort1(final float[] x, final short[] p, final int off, final int len) {
        // Insertion sort on smallest arrays
        if (len < 7) {
            for (int i = off; i < len + off; i++)
                for (int j = i; j > off && x[j - 1] < x[j]; j--)
        ...
        // Recursively sort non-partition-elements
        if ((s = b - a) > 1)
            reverseSort1(x, p, off, s);
        if ((s = d - c) > 1)
            reverseSort1(x, p, n - s, s);
    }
    ...
    /**
     * Searches the array of shorts for the specified value using the binary search algorithm. No sort order is assumed.
     * If the array contains duplicate copies of <code>key</code>, the location of the first occurrence will be
     * returned.
     * 
     * @param array the array to be searched
     * @param key the value to be searched for
     * @return index of the search key, if it is contained in the array; otherwise, {@link Integer#MIN_VALUE}
     */
    public static int linearSearch(final short[] array, final short key) {
        for (int i = 0; i < array.length; i++) {
            if (array[i] == key) {
                return i;
            }
