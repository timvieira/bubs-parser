...
public class Tokenizer {
	...
	public class Token {
		...
		public boolean isUnk() { return this.isUnk; }
	...
	public static String wordToUnkString(String word) {
		...
		// word case
		if (ParserUtil.isLowerCase(word)) unkStr += "-LC";
		else if (ParserUtil.isUpperCase(word)) unkStr += "-CAPS";
		else if (ParserUtil.isUpperCase(word.substring(0,1)) && ParserUtil.isLowerCase(word.substring(1))) unkStr += "-INITC";

		//if (lexSet.hasLabel(word.toLowerCase())) unkStr += "-KNOWNLC";
		if (ParserUtil.containsDigit(word)) unkStr += "-DIGIT";
		if (word.contains("-")) unkStr += "-DASH";
		
		String lcWord = word.toLowerCase();
		
		if (lcWord.endsWith("s") && !lcWord.endsWith("ss") && !lcWord.endsWith("us") && !lcWord.endsWith("is")) unkStr += "-s";
		if (lcWord.endsWith("ed")) unkStr += "-ed";
		if (lcWord.endsWith("ing")) unkStr += "-ing";
		if (lcWord.endsWith("ion")) unkStr += "-ion";
		if (lcWord.endsWith("er")) unkStr += "-er";
		if (lcWord.endsWith("est")) unkStr += "-est";
		if (lcWord.endsWith("al")) unkStr += "-al";
		
		if (lcWord.endsWith("ity")) unkStr += "-ity";
		else if (lcWord.endsWith("ly")) unkStr += "-ly";
		else if (lcWord.endsWith("y")) unkStr += "-y";
