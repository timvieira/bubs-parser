...
/**
 * Represents a Probabilistic Context Free Grammar (PCFG). Such grammars may be built up programatically or may be inferred from a corpus of data.
 * 
 * A PCFG consists of
 * <ul>
 ...
 * </ul>
 * 
 * Such grammars are useful in modeling and analyzing the structure of natural language or the (secondary) structure of many biological sequences.
 * 
 * Although branching of arbitrary size is possible in a grammar under this definition, this class models grammars which have been factored into binary-branching form, enabling
 * much more efficient computational approaches to be used.
 * 
 * Note - there will always be more productions than categories, but all categories except the special S-dagger category are also productions. The index of a category will be the
 * same when used as a production as it is when used as a category.
 * 
...
public class Grammar {
	...
	public float binaryLogProbability(final String parent, final String leftChild, final String rightChild) {
		if (nonTermSet.hasSymbol(parent) && nonTermSet.hasSymbol(leftChild) && nonTermSet.hasSymbol(rightChild)) {
			return binaryLogProbability(nonTermSet.getIndex(parent), nonTermSet.getIndex(leftChild), nonTermSet.getIndex(rightChild));
		}
