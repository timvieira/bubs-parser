...
/**
 * Sorted grammar implementation. Reads the grammar into memory and sorts non-terminals (V) according to their occurrence in binary rules. This can allow more efficient iteration
 * in grammar intersection (e.g., skipping NTs only valid as left children in the right cell) and more efficient chart storage (e.g., omitting storage for POS NTs in chart rows >=
 * 2).
 * 
 * @author Aaron Dunlop
 * @since Jun 6, 2010
...
public abstract class SortedGrammar extends GrammarByChild {
	...
	/**
	 * A temporary String -> String map, used to conserve memory while reading and sorting the grammar. Similar to {@link String}'s own intern map, but we don't need to internalize
	 * Strings indefinitely, so we map them ourselves and allow the map to be GC'd after we're done constructing the grammar.
	 */
	...
	/**
	 * Default Constructor. This constructor does an inordinate amount of work directly in the constructor specifically so we can initialize final instance variables. Making the
	 * instance vars final allows the JIT to inline them everywhere we use them, improving runtime efficiency considerably.
	 */
	protected SortedGrammar(final Reader grammarFile, final Reader lexiconFile, final GrammarFormatType grammarFormat) throws Exception {
		...
		// Initialize indices
		final int[] startAndEndIndices = comparator.startAndEndIndices(nonPosSet, leftChildrenSet, rightChildrenSet, pos);
	...
	public NonTerminal create(final String label, final HashSet<String> pos, final Set<String> nonPosSet, final Set<String> rightChildren) {
	...
	public abstract static class NonTerminalComparator implements Comparator<NonTerminal> {
		...
		/**
		 * @return an array containing leftChildStart, leftChildEnd, rightChildStart, rightChildEnd, posStart, posEnd, parentEnd
		 */
		public abstract int[] startAndEndIndices(HashSet<?> nonPosSet, HashSet<?> leftChildrenSet, HashSet<?> rightChildrenSet, HashSet<?> posSet);
	}
	...
	public static class PosLastComparator extends NonTerminalComparator {
		...
		@Override
		public int[] startAndEndIndices(final HashSet<?> nonPosSet, final HashSet<?> leftChildrenSet, final HashSet<?> rightChildrenSet, final HashSet<?> posSet) {
			...
			return new int[] { leftChildrenStart, leftChildrenEnd, rightChildrenStart, rightChildrenEnd, posStart, posEnd, parentEnd };
		}
	}
