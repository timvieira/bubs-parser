 ...
 * 
 * Implementations may store the binary rule matrix in a variety of formats (e.g. CSR ( {@link CsrSparseMatrixGrammar}) or CSC ({@link LeftCscSparseMatrixGrammar})).
 * 
 * The unary rules are always stored in CSR format, with rows denoting rule parents and columns rule children. We anticipate relatively few unary rules (the Berkeley grammar has
 * only ~100k), and iteration order doesn't matter greatly. CSC format would allow us to iterate through only the populated children, but CSR format makes parallelization much
 * simpler (e.g., we can execute one thread per row (parent); since we're only updating the parent probability, we do not need to synchronize with other threads).
 * 
 * @author Aaron Dunlop
 * @since Dec 31, 2009
 * 
...
public abstract class SparseMatrixGrammar extends SortedGrammar {
	...
	/**
	 * Column indices of each matrix entry in {@link #csrUnaryProbabilities}. One entry for each unary rule; the same size as {@link #csrUnaryProbabilities}.
	 */
	...
	public SparseMatrixGrammar(final Reader grammarFile, final Reader lexiconFile, final GrammarFormatType grammarFormat, Class<? extends CartesianProductFunction> functionClass)
			throws Exception {
	...
	public interface CartesianProductFunction {
		 ...
		 * @param leftChild
		 * @param rightChild
		 * @return packed representation of the specified child pair or {@link Integer#MIN_VALUE} if the pair is invalid.
		 */
