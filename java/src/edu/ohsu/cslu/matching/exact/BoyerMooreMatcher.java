...
/**
 * Implements the Boyer-Moore exact match algorithm. It's a very impressive algorithm, but limited profiling
 * indicates that in some cases this implementation can actually run slower than simple String.indexOf().
 * Possibly useful, but it's always hard to predict cache behavior.
 * 
        ...
        for (final String pattern : patterns) {
            ...
            while (k < textLength) {
                ...
                if (i < 0) {
                    ...
                    char c = textChars[h];
                    int badCharacter = Math
                        .max(1, i - (c >= preProcess.badc.length ? 0 : preProcess.badc[c]));
