...
/**
 * Implements the Baeza-Yates-Perlberg approximate match algorithm. This algorithm is very efficient
 * given a small number of edits in relation to the pattern length. For short patterns with many
 * edits allowed, the straight DP approaches may sometimes be more efficient.
 *
 * @author Aaron Dunlop
 * @since Jun 12, 2008
 *
...
public class BaezaYatesPerlbergMatcher extends ApproximateMatcher
{
    ...
    public Int2IntMap matchEditValues(Set<String> patterns, String text, int edits)
    {
        ...
        for (final String pattern : patterns)
        {
            ...
            final Set<Match> ahoCorasickMatchLocations = ahoCorasickMatcher.patternMatchLocations(subPatterns, text);
            ...
            for (Match matchLocation : ahoCorasickMatchLocations)
            {
                ...
                final Int2IntMap approximateMatchEditValues = dynamicMatcher.matchEditValues(pattern, text.substring(
                    b, e), edits);
        ...
        for (Iterator<Integer> i = matchEditValues.keySet().iterator(); i.hasNext();)
        {
            ...
            if ((matchEditValues.containsKey(matchLocation - 1) && matchEditValues.get(matchLocation - 1) < editValue)
                || (matchEditValues.containsKey(matchLocation + 1) && matchEditValues.get(matchLocation + 1) < editValue))
            {
                i.remove();
            }
        }
