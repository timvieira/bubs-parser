...
/**
 * Generic NaryTree implementation. The interface is defined recursively (children of an {@link NaryTree} are also {@link NaryTree}s, rather than defining an explicit Node class)
 * 
...
public class NaryTree<E> implements Tree<E>, Serializable {
	...
	protected NaryTree<E> addChild(final NaryTree<E> child) {
		child.parent = this;
	...
	public void addSubtree(final NaryTree<E> subtree) {
	...
	private void internalAddSubtree(final NaryTree<?> subtree) {
		addSubtree((NaryTree<E>) subtree);
	}
	...
	/**
	 * It's not technically possible to do an 'in-order' traversal of an n-ary tree, since there's no guaranteed position within the children in which the parent belongs. But some
	 * tree applications depend on branching in which the parent branches into a left branch and one or more right branches. This iterator follows that assumption and emits the
	 * parent node after the first child (if any) and before any other children.
	 * 
	 ...
	 */
	public NaryTree<E> subtree(final E childLabel) {
		for (final NaryTree<E> child : childList) {
			if (child.label.equals(childLabel)) {
				return child;
			}
	...
	public BinaryTree<String> factor(final GrammarFormatType grammarFormatType, final Factorization factorization) {
		...
		if (childList.size() > 0) {
			...
			case LEFT:
				...
				while (queue.size() > 1) {
					final BinaryTree<String> factoredTree = new BinaryTree<String>(grammarFormatType.factoredNonTerminal(label.toString()), factorization);
	 ...
	 */
	private void pqgramProfile(final int p, final int q, final PqgramProfile<E> profile, final NaryTree<E> r, ShiftRegister<E> anc) {
	 ...
	 *            The stream to read from
	 * @param labelParser
	 *            Parser appropriate for the labels contained in the tree
	 * @return the tree
	 * @throws IOException
	 *             if the read fails
	 */
	public static <T> NaryTree<T> read(final InputStream inputStream, final LabelParser<T> labelParser) throws IOException {
	 ...
	 *             if the read fails
	 */
	private static <T> NaryTree<T> readSubtree(final Reader reader, final NaryTree<T> parent, final LabelParser<T> labelParser) throws IOException {
