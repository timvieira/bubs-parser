...
public interface Matrix extends Serializable, Cloneable {
    ...
    /**
     * Returns a new matrix scaled by the provided multiplier. Note that a dense matrix is returned even if
     * the original matrix is sparse, since addition will populate previously empty entries.
     * 
     * @param addend
     ...
     * multiplication does not require populating previously empty entries, so the result may be a sparse
     * matrix.
     * 
     * @param multiplier
     * @return a new matrix scaled by the provided multiplier
    ...
    /**
     * @return true if this matrix is symmetric (that is, if its values are mirrored around the diagonal such
     *         that it is equal to its own transposition)
     */
    public boolean isSymmetric();

    /**
     * Not terribly useful mathematically, but useful as a simple single statistic about a matrix for unit
     * testing (at least until we bite the bullet and implement a determinant function). Note that we don't
     * test this method, since it's not really meaningful anyway.
     * 
    ...
    public static class Factory {
        ...
        /**
         * Returns a new {@link IntMatrix} of m rows x n columns, in which all cells are initialized to the
         ...
         * @param defaultValue
         * @param identityValue
         * @return an IntMatrix in which all cells such that i == j are pre-populated with identityValue and
         *         all others with defaultValue
         */
        public static IntMatrix newIdentityIntMatrix(final int m, final int defaultValue,
                final int identityValue) {
         ...
         * @param defaultValue
         * @param identityValue
         * @return a FloatMatrix in which all cells such that i == j are pre-populated with identityValue and
         *         all others with defaultValue
         */
         ...
         * @param defaultValue
         * @param identityValue
         * @return a symmetric FloatMatrix in which all cells such that i == j are pre-populated with
        ...
        public static Matrix read(final Reader reader) throws IOException {
            ...
            final boolean symmetric = attributes.containsKey(ATTRIBUTE_SYMMETRIC) ? Boolean
                .parseBoolean(attributes.get(ATTRIBUTE_SYMMETRIC)) : false;
