...
public abstract class IntMatrixTestCase extends MatrixTestCase {
    ...
    public void testVectorMultiply() throws Exception {
        ...
        assertEquals(new DenseIntVector(new int[] { 330, 780, 201 }), product);
        ...
        product = sampleMatrix.multiply(v);
        assertEquals(v.getClass(), product.getClass());
        assertEquals(new DenseFloatVector(new float[] { 330, 780, 201 }), product);

        // Multiply by a LargeSparseFloatVector
        v = new LargeSparseFloatVector(new float[] { 1, 2, 3, 4 });
        product = sampleMatrix.multiply(v);
        assertEquals(v.getClass(), product.getClass());
        assertEquals(new LargeSparseFloatVector(new float[] { 330, 780, 201 }), product);
    }
