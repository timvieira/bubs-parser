...
public abstract class FloatingPointMatrixTestCase extends MatrixTestCase {
    ...
    public void setUp() throws Exception {
        ...
        final float[][] symmetricArray = new float[][] { { 0f }, { 11.11f, 22.22f }, { 33.33f, 44.44f, 55.55f },
        ...
        final float[][] symmetricArray2 = new float[][] { { 0 }, { 0.1111f, 0.2222f }, { 0.3333f, 0.4444f, 0.5555f },
    ...
    public void testVectorMultiply() throws Exception {
        ...
        NumericVector v = new DenseIntVector(new int[] { 1, 2, 3, 4 });
        NumericVector product = sampleMatrix.multiply(v);
        assertEquals(DenseFloatVector.class, product.getClass());
        assertEquals(new DenseFloatVector(new float[] { 333.3f, 777.7f, 201.76f }), product);
        ...
        v = new DenseFloatVector(new float[] { 1, 2, 3, 4 });
        product = sampleMatrix.multiply(v);
        assertEquals(DenseFloatVector.class, product.getClass());
        assertEquals(new DenseFloatVector(new float[] { 333.3f, 777.7f, 201.76f }), product);

        // Multiply by a HashSparseFloatVector
        v = new LargeSparseFloatVector(new float[] { 1, 2, 3, 4 });
        product = sampleMatrix.multiply(v);
        assertEquals(LargeSparseFloatVector.class, product.getClass());
        assertEquals(new LargeSparseFloatVector(new float[] { 333.3f, 777.7f, 201.76f }), product);
    }
}
