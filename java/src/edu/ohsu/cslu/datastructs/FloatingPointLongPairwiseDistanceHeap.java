 ...
 * 
 * This implementation stores the distance as a 32-bit float and the two indices as unsigned 16-bit
 * integers. Since we only store positive distances between 0 and 1, it's safe to do ordinal
 * comparison using the bits of an IEEE floating-point representation.
 * 
...
public class FloatingPointLongPairwiseDistanceHeap extends BaseLongPairwiseDistanceHeap
{
    ...
    public void insert(int index1, int index2, float distance)
    {
        ...
        long l = ((long) Float.floatToIntBits(distance) << 32) | ((index1 & 0xffff) << 16) | (index2 & 0xffff);
