...
public class MatrixColumnAlignmentModel implements ColumnAlignmentModel {
    ...
    public MatrixColumnAlignmentModel(MultipleSequenceAlignment multipleSequenceAlignment,
            int[] pseudoCountsPerToken, int[] featureIndices, int emphasizedSequence, int additionalCounts,
            boolean[] binaryFeatures, float binaryFeatureGapCost) {
        ...
        for (int indexIntoFeatureIndices = 0; indexIntoFeatureIndices < featureCount; indexIntoFeatureIndices++) {
            ...
            counts[indexIntoFeatureIndices] = Matrix.Factory.newIntMatrix(
                newVocabularies[featureIndices[indexIntoFeatureIndices]].size(), multipleSequenceAlignment
                    .length(), pseudoCountsPerToken[indexIntoFeatureIndices]);
            ...
            if (sequence != null) {
                for (int columnIndex = 0; columnIndex < columns; columnIndex++) {
                    ...
                    for (int indexIntoFeatureIndeces = 0; indexIntoFeatureIndeces < featureCount; indexIntoFeatureIndeces++) {
                        counts[indexIntoFeatureIndeces].add(sequence.elementAt(columnIndex).getInt(
        ...
        for (int indexIntoFeatureIndices = 0; indexIntoFeatureIndices < featureCount; indexIntoFeatureIndices++) {
            ...
            for (int i = 0; i < matrix.rows(); i++) {
                if (binaryFeatures[indexIntoFeatureIndices] && i == AlignmentModel.GAP_INDEX) {
                    ...
                    for (int j = 0; j < matrix.columns(); j++) {
                        matrix.set(i, j, calculateNegativeLogP(
     ...
     * 
     * sum over all features: a) cost of aligning the NEW feature-value of feature[i] to a gap TIMES b) total
     * cost of ripping a gap into all already-aligned sequence
     * 
     * Where b) is defined as the same as aligning a featureVector to a slot that contains zero matching
     * feature values.
     * 
     * @param featureVector
     *            -- the values of all features for one slot of the NEW sequence to align
