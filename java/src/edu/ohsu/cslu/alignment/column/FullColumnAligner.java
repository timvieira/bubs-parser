...
public class FullColumnAligner extends BaseColumnAligner {
    ...
    @Override
    public SequenceAlignment alignWithGaps(MappedSequence sequence, MatrixColumnAlignmentModel model,
            int[] features) {
        final int maxI = sequence.length() + 1;
        final int maxJ = model.columnCount() + 1;
        ...
        for (int i = 1; i < maxI; i++) {
            scoreArray[i][0] = scoreArray[i - 1][0]
                    + model.costOfInsertingAGapIntoThisAlignmentModel/* _reflexive */(sequence
                        .elementAt(i - 1));
            // scoreArray[i][0] = scoreArray[i - 1][0] +
    ...
    protected final SequenceAlignment backtrace(Sequence sequence, ColumnAlignmentModel model,
        ...
        if (sequence instanceof MultipleVocabularyMappedSequence) {
            return new SequenceAlignment(new MultipleVocabularyMappedSequence(backtrace
                .toArray(new IntVector[backtrace.size()]), model.vocabularies()), gapIndices, score);
        }
        return new SequenceAlignment(new LogLinearMappedSequence(backtrace.toArray(new BitVector[backtrace
            .size()]), model.vocabularies()[0]), gapIndices, score);
    }
    ...
    public String toString() {
        ...
        for (int i = 0; i < maxI; i++) {
            ...
            for (int j = 0; j < maxJ; j++) {
                ...
                sb.append(String
                    .format(value > 500000 ? " %s      Max |" : " %s %8.2f |", backpointer, value));
            }
            sb.append('\n');
        }
        return sb.toString();
