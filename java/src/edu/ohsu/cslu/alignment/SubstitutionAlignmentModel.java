...
/**
 * Represents a substitution-cost based alignment model, in which each substitution, deletion, or gap
 * insertion is penalized by a particular cost.
 * 
 * Implementations may use hand-tuned or induced cost matrices or (potentially) more complex cost functions.
 * 
 * @author Aaron Dunlop
 * @since Oct 7, 2008
 * 
...
public interface SubstitutionAlignmentModel extends AlignmentModel {
    ...
    /**
     * Returns the cost of inserting a gap in an alignment of the specified length. The gap insertion cost is
     * the same when inserting a gap into an already-aligned sequence (in which case the relevant length is
     * that of the existing alignment) or when inserting the gap into the unaligned sequence (in which case
     * the relevant length is that of the unaligned sequence).
     * 
    ...
    /**
     * Returns the cost of inserting a gap for the featureValue featureValueIndex in the feature featureIndex
     * 
     * @param featureIndex
