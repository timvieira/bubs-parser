...
public class TestMultipleSequenceAligners {
    ...
    public static void suiteSetUp() throws Exception {
        ...
        final DenseMatrix matrix0 = Matrix.Factory.newIdentityIntMatrix(linguisticVocabularies[0].size(), 10,
            0);
        matrix0.setRow(0, 4);
        matrix0.setColumn(0, 4);

        final DenseMatrix matrix1 = Matrix.Factory.newIdentityIntMatrix(linguisticVocabularies[1].size(), 1,
            0);
        matrix1.setRow(0, 4);
        matrix1.setColumn(0, 4);

        linguisticAlignmentModel = new MatrixSubstitutionAlignmentModel(
            new DenseMatrix[] { matrix0, matrix1 }, linguisticVocabularies);
    }

    @Test
    public void testDnaAlignment() throws Exception {
        final Matrix levenshteinDistances = CalculateDistances.LevenshteinDistanceCalculator
            .distances(SAMPLE_SEQUENCES);
        final MappedSequence[] sampleSequences2 = DNA_VOCABULARY.mapSequences(SAMPLE_SEQUENCES);

        // Full dynamic aligner
        final MultipleSequenceAligner aligner = new IterativePairwiseAligner();

        // Hand-tuned matrix designed to force more gap insertion
        final DenseMatrix matrix2 = new IntMatrix(new int[][] { { 3, 99, 99, 98, 99, 200 },
                { 93, 26, 95, 90, 94, 500 }, { 93, 94, 29, 92, 89, 500 }, { 93, 91, 94, 25, 94, 500 },
                { 92, 92, 87, 91, 34, 500 }, { 92, 75, 80, 69, 81, 500 } });
        final AlignmentModel model2 = new MatrixSubstitutionAlignmentModel(matrix2, DNA_VOCABULARY);
        final MultipleSequenceAlignment alignedSequences = aligner.align(sampleSequences2,
    ...
    public void testLinguisticAlignment() throws Exception {
        final MappedSequence sequence1 = new MultipleVocabularyMappedSequence(Strings
            .extractPos(sampleSentence4), linguisticVocabularies);
        final MappedSequence sequence2 = new MultipleVocabularyMappedSequence(Strings
            .extractPos(sampleSentence16), linguisticVocabularies);
        final MappedSequence sequence3 = new MultipleVocabularyMappedSequence(Strings
            .extractPos(sampleSentence8), linguisticVocabularies);

        final MultipleSequenceAligner aligner = new IterativePairwiseAligner();
        Matrix distanceMatrix = new IntMatrix(new int[][] { { 0, 1 }, { 1, 0 } });
        MultipleSequenceAlignment sequenceAlignment = aligner.align(new MappedSequence[] { sequence1,
                sequence2 }, distanceMatrix, linguisticAlignmentModel);
        ...
        sequenceAlignment = aligner.align(new MappedSequence[] { sequence1, sequence2, sequence3 },
