...
/**
 * Represents a multiple-sequence-alignment; that is, a bag of sequences with gaps inserted as needed in order
 * to align them with one another.
 * 
 * This implementation uses a simplistic ArrayList representation. When aligning large sets of long sequences,
 * it can be more efficient to represent the sequence bag with more sophisticated data structures, such as a
 * Linked List of arrays, allowing for efficient gap insertion.
 * 
 * @author Aaron Dunlop
 * @since Oct 11, 2008
 * 
...
public class MultipleSequenceAlignment implements Serializable {
    ...
    /**
     * Adds a sequence to this alignment at a specified location (useful when we want the final alignment in
     * the same order as the original list of sequences)
     * 
     * @param newSequence
     * @param index
     */
    ...
    private void checkSequence(MappedSequence newSequence) {
        ...
        if (newSequence.featureCount() != featureCount) {
            throw new IllegalArgumentException(String.format(
                "Sequence feature count (%d) does not match alignment feature count (%d)", newSequence
    ...
    public ColumnAlignmentModel inducePssmAlignmentModel(int[] pseudoCountsPerToken, int[] featureIndices,
        ...
        for (int f = 0; f < featureCount; f++) {
            ...
            totalCounts[f] = pseudoCountsPerToken[f] * newVocabularies[featureIndices[f]].size()
                    + numOfSequences() + additionalCounts;
        }
    ...
    /**
     * Induces a {@link LogLinearAlignmentModel} from the sequence. Alignment cost vectors for each column are
     * estimated using maximum likelihood with Laplace smoothing.
     * 
     * @param laplacePseudoCounts
     *            Laplace smoothing counts for each vocabulary entry
     * @param scalingVector
     *            Weights for each vocabulary entry. If not null, the alignment cost vectors are scaled by
     *            this vector (if null, the final alignment vectors are left un-scaled).
    ...
    public LogLinearAlignmentModel induceLogLinearAlignmentModel(NumericVector laplacePseudoCounts,
        ...
        if (totalCountVector == null || !laplacePseudoCounts.equals(cachedLaplacePseudoCounts)) {
            ...
            for (int i = 0; i <= categoryBoundaries.length + 1; i++) {
                final int nextBoundary = i < categoryBoundaries.length ? categoryBoundaries[i] : vocabulary
                    .size();
                final float laplaceTotalCount = laplacePseudoCounts.subVector(previousBoundary,
                    nextBoundary - 1).sum();
    ...
    /**
     * Reads a character-based sequence alignment, such as a DNA, RNA, or protein alignment in the format:
     * 
     * <pre>
