...
public class TestMultipleSequenceAlignment {
    ...
    @Test
    public void testGet() {
        MultipleSequenceAlignment linguisticAlignment = new MultipleSequenceAlignment(new MappedSequence[] {
                linguisticSequence1, linguisticSequence2 });
    ...
    public void testInsertGaps() {
        ...
        assertEquals(linguisticSequence1.insertGaps(gapIndices).toString(), linguisticAlignment.get(0)
            .toString());
        assertEquals(linguisticSequence2.insertGaps(gapIndices).toString(), linguisticAlignment.get(1)
            .toString());
    }
    ...
    public void testInducePssmAlignmentModel() throws IOException {
        ...
        assertEquals(Float.POSITIVE_INFINITY, pssmModel.cost(new IntVector(new int[] {
                posVocabulary.map("DT"), wordVocabulary.map("fall") }), 0), .01f);
        assertEquals(Float.POSITIVE_INFINITY, pssmModel.cost(new IntVector(new int[] {
                posVocabulary.map("NN"), wordVocabulary.map("The") }), 0), .01f);
        ...
        assertEquals(Float.POSITIVE_INFINITY, pssmModel.cost(new IntVector(new int[] {
                posVocabulary.map("DT"), wordVocabulary.map("_-") }), 0), .01f);
        assertEquals(Float.POSITIVE_INFINITY, pssmModel.cost(new IntVector(new int[] {
                posVocabulary.map("_-"), wordVocabulary.map("The") }), 0), .01f);
        ...
        assertEquals(-Math.log(1f / 9), pssmModel.cost(new IntVector(new int[] { posVocabulary.map("TO"),
                wordVocabulary.map("are") }), 3), .01f);
        assertEquals(Float.POSITIVE_INFINITY, pssmModel.cost(new IntVector(new int[] {
                posVocabulary.map("DT"), wordVocabulary.map("are") }), 3), .01f);
    ...
    public void testInduceLogLinearAlignmentModel() throws IOException {
        ...
        // First test a maximum-likelihood model
        ColumnAlignmentModel pssmModel = alignment.induceLogLinearAlignmentModel(new FloatVector(
            dnaVocabulary.size(), 0), null, dnaColumnInsertionCostVector);
        ...
        FloatVector laplacePseudoCounts = new FloatVector(new float[] { 1f, 1f / 4, 1f / 4, 1f / 4, 1f / 4,
                1, 1, 1, 0 });
        ...
        SparseBitVector theDT = new SparseBitVector(new int[] { vocabulary.map("the"),
                vocabulary.map("_pos_DT") }, false);
        ...
        SparseBitVector ranVBN = new SparseBitVector(new int[] { vocabulary.map("ran"),
                vocabulary.map("_pos_VBN"), vocabulary.map("_head_verb") }, false);
    ...
    public void testToString() {
        ...
        sb
            .append(" _- |       NN | AUX |  TO |       VB | IN |   _- |    _- |             JJ |     _- |   CD | . |\n");
        sb
            .append(" _- | Delivery |  is |  to |    begin | in |   _- |    _- |          early |     _- | 1991 | . |\n");
        sb
            .append("------------------------------------------------------------------------------------------------\n");
        sb
            .append(" _- |      JJS |  _- | AUX |      VBN | TO |   VB |    IN |             JJ |    NNS |   _- | . |\n");
        sb
