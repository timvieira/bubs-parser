 ...
 * 
 * The responsibilities of an {@link AlignmentModel} is to assign a cost to aligning a particular token (or a
 * set of features representing a token) at each point in the dynamic alignment process.
 * 
 * For a column-based alignment (i.e., when aligning a new {@link Sequence} with an existing MSA), that cost
 * will depend on the feature vector and on the column index. (see {@link ColumnAlignmentModel}).
 * 
 * For a pairwise alignment (i.e., when aligning one {@link Sequence} with another), that cost will depend on
 * comparing a pair of feature vectors from the two sequences. (see {@link SubstitutionAlignmentModel}).
 * 
 * @author Aaron Dunlop
 * @since Oct 10, 2008
 * 
