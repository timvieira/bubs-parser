...
/**
 * The simplest possible implementation of the {@link Sequence} interface. Represents a sequence as a simple
 * int[] array, mapped using a {@link Vocabulary}
 * 
 * We're using a matrix of m tokens, each with n features, in hopes of better cache performance during
 * processing (since we'll usually be iterating over the length of the sequence and touching each feature at
 * each index)
 * 
 * @author Aaron Dunlop
 * @since Oct 9, 2008
...
public final class MultipleVocabularyMappedSequence implements MappedSequence, Cloneable, Serializable {
     ...
     * @param vocabularies
     */
    public MultipleVocabularyMappedSequence(final String bracketedSequence,
    ...
    /**
     * @param index
     *            position in the {@link Sequence}
     * @param featureIndex
     *            feature / {@link Vocabulary}
     * @return integer representation of the token at the specified index, mapped using the {@link Vocabulary}
     *         of the specified <code>featureIndex</code>
     */
    ...
    /**
     * @param index
     * @return integer representation of the token at the specified index. Each integer represents a feature
     *         mapped using the {@link Vocabulary} of the same index.
     */
    ...
    public Sequence subSequence(final int beginIndex, final int endIndex) {
        ...
        return new MultipleVocabularyMappedSequence(matrix.subMatrix(beginIndex, endIndex - 1, 0, matrix
            .columns() - 1), vocabularies);
    }
