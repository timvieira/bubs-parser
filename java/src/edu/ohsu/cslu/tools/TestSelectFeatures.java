...
public class TestSelectFeatures extends ToolTestCase {
    ...
    public void testBracketedInput() throws Exception {
        ...
        assertEquals("(<s>) (Ms.) (Haag) (plays) (Elianti) (.) (</s>)\n", executeTool(new SelectFeatures(), "-i bracketed -f 1", oneLineInput));
        assertEquals("(<s>) (Ms.) (Haag) (plays) (Elianti) (.) (</s>)\n", executeTool(new SelectFeatures(), "-i bracketed -f 1 -o bracketed", oneLineInput));
        assertEquals("[<s>] [Ms.] [Haag] [plays] [Elianti] [.] [</s>]\n", executeTool(new SelectFeatures(), "-f 1 -o square-bracketed", oneLineInput));
        assertEquals("<s> Ms. Haag plays Elianti . </s>\n", executeTool(new SelectFeatures(), "-f 1 -o stanford", oneLineInput));

        // Select two features
        assertEquals("(<s> before_head) (Ms. before_head) (Haag before_head) (plays head_verb) (Elianti after_head) (. after_head) (</s> after_head)\n", executeTool(
                new SelectFeatures(), "-i bracketed -f 1,3", oneLineInput));
        ...
        assertEquals("<s>/<s> NNP/Ms. NNP/Haag VBZ/plays NNP/Elianti ./. </s>/</s>\n", executeTool(new SelectFeatures(), "-f 2,1 -o stanford", oneLineInput));
        ...
        final String twoLineInput = "(RB Sometimes before_head) (PRP they before_head) (AUX are head_verb) (JJ constructive after_head)"
                ...
                + " (VBD overturned head_verb) (NNP Mr. after_head) (NNP Blair after_head) (POS 's after_head) (NN decision after_head)" + " (. . after_head)";

        // Select only the word
        assertEquals("(Sometimes) (they) (are) (constructive)" + " (,) (but) (often) (not) (.)\n" + "(The) (Ontario) (Supreme) (Court)"
                + " (overturned) (Mr.) (Blair) ('s) (decision)" + " (.)\n", executeTool(new SelectFeatures(), "-f 2", twoLineInput));

        // Select all features
        assertEquals("(RB Sometimes before_head) (PRP they before_head) (AUX are head_verb) (JJ constructive after_head) (, , after_head)"
                + " (CC but after_head) (RB often after_head) (RB not after_head) (. . after_head)\n"
                + "(DT The before_head) (NNP Ontario before_head) (NNP Supreme before_head) (NNP Court before_head)"
                + " (VBD overturned head_verb) (NNP Mr. after_head) (NNP Blair after_head) (POS 's after_head) (NN decision after_head)" + " (. . after_head)\n", executeTool(
                new SelectFeatures(), "-f 1,2,3", twoLineInput));
    }

    /**
     * Tests selecting features from POS-tagged, bracketed input
     * 
     * @throws Exception if something bad happens
     */
    @Test
    public void testPosTaggedInput() throws Exception {
        final String input = "(<s> <s> f3_1) (NNP Ms. f3_2) (NNP Haag f3_3) (VBZ plays f3_4) (NNP Elianti f3_5) (. . f3_6) (</s> </s> f3_7)";

        assertEquals("(<s> f3_1) (Ms. f3_2) (Haag f3_3) (plays f3_4) (Elianti f3_5) (. f3_6) (</s> f3_7)\n", executeTool(new SelectFeatures(), "-i pos-tagged -w -f 3", input));
        assertEquals("(<s> _pos_<s>) (Ms. _pos_NNP) (Haag _pos_NNP) (plays _pos_VBZ) (Elianti _pos_NNP) (. _pos_.) (</s> _pos_</s>)\n", executeTool(new SelectFeatures(),
                "-i pos-tagged -w -p", input));
        assertEquals("(<s>) (Ms. _pos-1_<s>) (Haag _pos-1_NNP) (plays _pos-1_NNP) (Elianti _pos-1_VBZ) (. _pos-1_NNP) (</s> _pos-1_.)\n", executeTool(new SelectFeatures(),
                "-i pos-tagged -w -prevpos 1", input));
    }
    ...
    public void testTreeInput() throws Exception {
        ...
        final String twoLineInput = "(TOP (S (S (ADVP (RB Sometimes)) (NP (PRP they)) (VP (AUX are)"
                + " (ADJP (JJ constructive)))) (, ,) (CC but) (FRAG (ADVP (RB often)) (RB not)) (. .)))\n"
                + "(TOP (S (NP (DT The) (NNP Ontario) (NNP Supreme) (NNP Court)) (VP (VBD overturned)" + " (NP (NP (NNP Mr.) (NNP Blair) (POS 's)) (NN decision))) (. .)))";
        ...
        assertEquals("(sometimes _pos_RB _before_head) (they _pos_PRP _before_head) (are _pos_AUX _head_verb) (constructive _pos_JJ _after_head)"
                + " (, _pos_, _after_head) (but _pos_CC _after_head) (often _pos_RB _after_head) (not _pos_RB _after_head) (. _pos_. _after_head)\n", executeTool(
                new SelectFeatures(), "-i bracketed-tree -lcw -p -h -bh -ah", oneLineInput));

        // Test extracting previous 2 words, subsequent 1 word
        assertEquals("(sometimes _word+1_they) (they _word-1_sometimes _word+1_are) (are _word-1_they _word-2_sometimes _word+1_constructive)"
                ...
                + " (not _word-1_often _word-2_but _word+1_.) (. _word-1_not _word-2_often)\n", executeTool(new SelectFeatures(), "-lcw -i bracketed-tree -prevword 2 -subword 1",
                oneLineInput));

        // Test extracting previous word, previous and subsequent POS
        assertEquals("(sometimes _pos+1_PRP) (they _word-1_sometimes _pos-1_RB _pos+1_AUX) (are _word-1_they _pos-1_PRP _pos+1_JJ)"
                + " (constructive _word-1_are _pos-1_AUX _pos+1_,) (, _word-1_constructive _pos-1_JJ _pos+1_CC)"
                + " (but _word-1_, _pos-1_, _pos+1_RB) (often _word-1_but _pos-1_CC _pos+1_RB)" + " (not _word-1_often _pos-1_RB _pos+1_.) (. _word-1_not _pos-1_RB)\n",
                executeTool(new SelectFeatures(), "-lcw -i bracketed-tree -prevword 1 -prevpos 1 -subpos 1", oneLineInput));

        // Select only the head features
        assertEquals("(_before_head) (_before_head) (_head_verb) (_after_head) (_after_head) (_after_head) (_after_head) (_after_head) (_after_head)\n", executeTool(
                new SelectFeatures(), "-i tree -h -bh -ah", oneLineInput));
        assertEquals("_before_head _before_head _head_verb _after_head _after_head _after_head _after_head _after_head _after_head\n", executeTool(new SelectFeatures(),
                "-i tree -h -bh -ah -o stanford", oneLineInput));

        // And from a square-bracketed tree
        assertEquals("(sometimes _pos_RB _before_head) (they _pos_PRP _before_head) (are _pos_AUX _head_verb) (constructive _pos_JJ _after_head)"
                + " (, _pos_, _after_head) (but _pos_CC _after_head) (often _pos_RB _after_head) (not _pos_RB _after_head) (. _pos_. _after_head)\n", executeTool(
                new SelectFeatures(), "-i square-bracketed-tree -lcw -p -h -bh -ah", oneLineInput.replaceAll("\\(", "[").replaceAll("\\)", "]")));

        // Extract only word and head-verb feature from two lines, outputting a square-bracketed
        // format
        assertEquals("[sometimes _before_head] [they _before_head] [are _head_verb] [constructive _after_head] [, _after_head]"
                ...
                "-i tree -lcw -h -bh -ah -o square-bracketed", twoLineInput));
        ...
        assertEquals("sometimes/RB they/PRP are/AUX constructive/JJ ,/, but/CC often/RB not/RB ./.\n"
                ...
                "-i tree -lcw -ppos -o stanford", twoLineInput));
    }

    /**
     * Tests running the tool with multiple threads. Not a very useful scenario for {@link SelectFeatures}, but a reasonable test for the threading of
