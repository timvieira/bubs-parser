 ...
 * implementation of the {@link Filter} interface is created based on the system property
 * 'junit.filter.class' and filters individual test methods at runtime. If the system property is
 * not specified, {@link DefaultFilter} is created, which runs all tests except for those labeled as
 * performance tests.
 * 
 * Tests which are filtered out are marked as ignored. Similar functionality could be achieved using
 * JUnit assumptions, but a failed assumption is marked as a successful test, which seems odd, and
 * hides the fact that the test did not actually execute.
 * 
 * @author aarond
 * @since Dec 30, 2008
 * 
...
public class FilteredRunner extends BlockJUnit4ClassRunner
{
    ...
    /**
     * This implementation should work, but there's a bug in the JUnit test runner that causes
     * ignored classes to be marked as 'Unrooted'. So we'll just work around it in runChild()
     */
    // @Override
    ...
    {
        /*
         * TODO: This checks the class on each test method, and marks the methods as ignored instead
         * of the entire class.
         * 
         * Overriding run() should allow us to mark the entire class as ignored when appropriate,
         * but this seems to result in classes labeled as 'Unrooted Tests' instead of marking them
         * the same as classes annotated with @Ignore. It appears this is a bug in the Eclipse JUnit
         * runner, but that's an important one, so we'll work around it this way.
         */

        Description classDescription = Description.createSuiteDescription(getTestClass().getJavaClass().getName(),
            getTestClass().getJavaClass().getAnnotations());
        ...
        else
        {
            ...
            else
            {
                ...
                if (expectedTime == 0)
                {
                    notifier.fireTestFailure(new Failure(methodDescription, new AssertionFailedError(String.format(
                        "No time annotated for test hardware '%s'", testHardware))));
                }
            }
            ...
            {
                notifier.fireTestFailure(new Failure(methodDescription, new AssertionFailedError(String.format(
                    "Expected elapsed time within 20%% of %d but was %d instead", expectedTime, elapsedTime))));
            }
