...
/**
 * Extracts features from a {@link BinaryTagSequence}, using the feature templates from {@link TaggerFeatureExtractor}.
 * 
 * @author Aaron Dunlop
 * @since Jul 25, 2013
 */
public class BinaryTaggerFeatureExtractor extends TaggerFeatureExtractor<BinaryTagSequence> {
    ...
    @Override
    public BitVector featureVector(final BinaryTagSequence sequence, final int position) {
        ...
        for (int i = 0; i < templates.length; i++) {
            ...
            for (int j = 0; j < template.length; j++) {
                ...
                case tm3:
                    feature *= tagSetSize;
                    feature += ((index < 0 || index >= sequence.predictedClasses.length) ? 2
                            : sequence.predictedClasses[index] ? 1 : 0); // False = 0, True = 1, Null = 2
                    break;
