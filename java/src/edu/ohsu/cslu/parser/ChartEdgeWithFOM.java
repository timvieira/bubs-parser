...
public class ChartEdgeWithFOM extends ChartEdge implements Comparable<ChartEdgeWithFOM> {
    ...
    public ChartEdgeWithFOM(final Production p, final BaseChartCell leftCell, final BaseChartCell rightCell, final float insideScore, final EdgeFOM edgeFOM,
            final ChartParser parser) {
        super(p, leftCell, rightCell, insideScore);
        this.figureOfMerit = edgeFOM.calcFOM(this, parser);
    }

    public ChartEdgeWithFOM(final Production p, final BaseChartCell childCell, final float insideScore, final EdgeFOM edgeFOM, final ChartParser parser) {
