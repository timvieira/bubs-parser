...
public class BSCPPruneViterbiStats extends BSCPPruneViterbi {
    ...
    public BSCPPruneViterbiStats(final ParserOptions opts, final LeftHashGrammar grammar) {
        super(opts, grammar);
    }
    ...
    protected void addEdgeCollectionToChart(final HashSetChartCell cell) {
        ...
        if (nGoldEdges > 0) {
            System.out.println("DSTAT: " + this.chart.size() + " " + cell.width() + " " + nGoldEdges + " "
                    + goldRank + " " + nPopBinary + " " + nPopUnary + " " + (nPopBinary + nPopUnary) + " ");
        }
    }
}
