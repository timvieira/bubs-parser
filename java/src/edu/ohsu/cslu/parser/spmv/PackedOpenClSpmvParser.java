...
public class PackedOpenClSpmvParser extends OpenClSpmvParser<PackedArrayChart> {
	...
	/**
	 * Populated non-terminal indices. Parallel array with {@link OpenClSpmvParser#clChartInsideProbabilities} , {@link OpenClSpmvParser#clChartPackedChildren}, and
	 * {@link OpenClSpmvParser#clChartMidpoints}
	 */
	...
	@Override
	protected void internalCartesianProduct(final ParallelArrayChartCell leftCell, final ParallelArrayChartCell rightCell, final CLFloatBuffer tmpClCartesianProductProbabilities,
		...
		cartesianProductKernel.setArgs(clChartNonTerminalIndices, clChartInsideProbabilities, clChartPackedChildren, clChartMidpoints, leftChildrenStart, observedLeftChildren,
				rightChildrenStart, observedRightChildren, tmpClCartesianProductProbabilities, tmpClCartesianProductMidpoints, (short) rightCell.start());

		// Call the kernel and wait for results
		final int globalWorkSize = edu.ohsu.cslu.util.Math.roundUp((observedLeftChildren * observedRightChildren), LOCAL_WORK_SIZE);
		if (globalWorkSize > 0) {
	...
	protected void internalBinarySpmvMultiply(final ParallelArrayChartCell chartCell) {
		...
		binarySpmvKernel.setArgs(clTmpCellInsideProbabilities, clTmpCellPackedChildren, clTmpCellMidpoints, clCartesianProductProbabilities0, clCartesianProductMidpoints0,
				clBinaryRuleMatrixRowIndices, clBinaryRuleMatrixColumnIndices, clBinaryRuleMatrixProbabilities, grammar.numNonTerms());
	...
	protected void internalUnarySpmvMultiply(final ParallelArrayChartCell chartCell) {
		...
		// Bind the arguments of the OpenCL kernel
		unarySpmvKernel.setArgs(clTmpCellInsideProbabilities, clTmpCellPackedChildren, clTmpCellMidpoints, clCsrUnaryRowStartIndices, clCsrUnaryColumnIndices,
	...
	protected void finalizeCell(final ParallelArrayChartCell chartCell) {
		...
		prefixSumKernel.setArgs(clTmpCellInsideProbabilities, clPrefixSum, clChartNumNonTerminals, chart.cellIndex(packedCell.start(), packedCell.end()), grammar.numNonTerms());
		// TODO Thread prefix sum kernel
		prefixSumKernel.enqueueNDRange(clQueue, new int[] { 1 }, new int[] { 1 });

		packKernel.setArgs(clTmpCellInsideProbabilities, clTmpCellPackedChildren, clTmpCellMidpoints, clPrefixSum, clChartNonTerminalIndices, clChartInsideProbabilities,
				clChartPackedChildren, clChartMidpoints, chartCell.offset(), grammar.numNonTerms());
