 ...
 */
public abstract class OpenClSpmvParser<C extends ParallelArrayChart> extends SparseMatrixVectorParser<CsrSparseMatrixGrammar, C> {
	...
	protected OpenClSpmvParser(final ParserDriver opts, final CsrSparseMatrixGrammar grammar) {
		...
		try {
			...
			prefix.write("#define MAX_PACKED_LEXICAL_PRODUCTION " + ((LeftShiftFunction) grammar.cartesianProductFunction()).maxPackedLexicalProduction + '\n');
			prefix.write("#define PACKING_SHIFT " + ((LeftShiftFunction) grammar.cartesianProductFunction()).shift + '\n');
			...
			final CLProgram clSharedProgram = OpenClUtils.compileClKernels(context, OpenClSpmvParser.class, prefix.toString());
		...
		clBinaryRuleMatrixRowIndices = OpenClUtils.copyToDevice(clQueue, grammar.csrBinaryRowIndices, CLMem.Usage.Input);
		clBinaryRuleMatrixColumnIndices = OpenClUtils.copyToDevice(clQueue, grammar.csrBinaryColumnIndices, CLMem.Usage.Input);
		clBinaryRuleMatrixProbabilities = OpenClUtils.copyToDevice(clQueue, grammar.csrBinaryProbabilities, CLMem.Usage.Input);

		// Repeat for unary rules
		clCsrUnaryRowStartIndices = OpenClUtils.copyToDevice(clQueue, grammar.csrUnaryRowStartIndices, CLMem.Usage.Input);
		...
		clCartesianProductProbabilities0 = context.createFloatBuffer(CLMem.Usage.InputOutput, grammar.cartesianProductFunction().packedArraySize());
		clCartesianProductMidpoints0 = context.createShortBuffer(CLMem.Usage.InputOutput, grammar.cartesianProductFunction().packedArraySize());
		clCartesianProductProbabilities1 = context.createFloatBuffer(CLMem.Usage.InputOutput, grammar.cartesianProductFunction().packedArraySize());
		clCartesianProductMidpoints1 = context.createShortBuffer(CLMem.Usage.InputOutput, grammar.cartesianProductFunction().packedArraySize());
	}
	...
	/**
	 * De-allocates current OpenCL chart storage (if any) and allocates storage adequate for the current sentence length.
	 */
	 ...
	 * copies the chart into device memory before taking the cartesian product and copies the result back into main memory. Primarily to enable unit testing of
	...
	protected CartesianProductVector cartesianProductUnion(final int start, final int end) {
		...
		final float[] probabilities = OpenClUtils.copyFromDevice(clQueue, clCartesianProductProbabilities0, packedArraySize);
	...
	protected void internalCartesianProductUnion(final int start, final int end) {
		...
		fillFloatKernel.setArgs(clCartesianProductProbabilities0, grammar.cartesianProductFunction().packedArraySize(), Float.NEGATIVE_INFINITY);
		final int globalWorkSize = edu.ohsu.cslu.util.Math.roundUp(grammar.cartesianProductFunction().packedArraySize(), LOCAL_WORK_SIZE);
		...
		internalCartesianProduct(chart.getCell(start, start + 1), chart.getCell(start + 1, end), clCartesianProductProbabilities0, clCartesianProductMidpoints0);
		...
		for (short midpoint = (short) (start + 2); midpoint <= end - 1; midpoint++) {
			...
			fillFloatKernel.setArgs(clCartesianProductProbabilities1, grammar.cartesianProductFunction().packedArraySize(), Float.NEGATIVE_INFINITY);
			fillFloatKernel.enqueueNDRange(clQueue, new int[] { globalWorkSize }, new int[] { LOCAL_WORK_SIZE });
			clQueue.finish();

			// Perform the cartesian product
			internalCartesianProduct(leftCell, rightCell, clCartesianProductProbabilities1, clCartesianProductMidpoints1);

			t1 = System.currentTimeMillis();
			totalCartesianProductTime += (t1 - t0);

			// Union the new cross-product with the existing cross-product
			cartesianProductUnionKernel.setArgs(clCartesianProductProbabilities0, clCartesianProductMidpoints0, clCartesianProductProbabilities1, clCartesianProductMidpoints1,
					grammar.cartesianProductFunction().packedArraySize());
			cartesianProductUnionKernel.enqueueNDRange(clQueue, new int[] { globalWorkSize }, new int[] { LOCAL_WORK_SIZE });
			clQueue.finish();
	 ...
	 */
	protected abstract void internalCartesianProduct(final ParallelArrayChartCell leftCell, final ParallelArrayChartCell rightCell,
	 ...
	 * 
	 * This version copies the cross-product to device memory and the resulting chart cell back into main memory. Primarily for unit testing of
	 * {@link #internalBinarySpmvMultiply(ParallelArrayChartCell)} .
	 */
	@Override
	 ...
	 * 
	 * This version copies the current cell population to device memory and the results back into main memory. Primarily for unit testing of
	 * {@link #internalUnarySpmvMultiply(ParallelArrayChartCell)}.
	 */
	@Override
