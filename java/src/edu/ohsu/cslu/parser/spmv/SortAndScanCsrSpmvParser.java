...
public class SortAndScanCsrSpmvParser extends SparseMatrixVectorParser<CsrSparseMatrixGrammar, PackedArrayChart> {
	...
	public SortAndScanCsrSpmvParser(final CsrSparseMatrixGrammar grammar) {
		super(ParserDriver.defaultTestOptions(), grammar);
	}
	...
	protected CartesianProductVector cartesianProductUnion(final int start, final int end) {
		...
		for (short midpoint = (short) (start + 1); midpoint <= end - 1; midpoint++) {
			...
			for (int i = leftCell.minLeftChildIndex(); i <= leftCell.maxLeftChildIndex(); i++) {
				...
				for (int j = rightCell.offset(); j <= rightCell.maxRightChildIndex(); j++) {
					...
					final int cartesianProductIndex = offsets[midpoint - start] + leftChildrenProcessed * rightCellRightChildren + rightChildrenProcessed;

					tmpCartesianProductChildren[cartesianProductIndex] = grammar.cartesianProductFunction().pack(leftChildIndex, nonTerminalIndices[j]);
					tmpCartesianProductInsideProbabilities[cartesianProductIndex] = leftProbability + insideProbabilities[j];
		...
		sorter.sort(tmpCartesianProductChildren, tmpCartesianProductInsideProbabilities, tmpCartesianProductMidpoints, offsets);
		...
		scanner.parallelArrayInclusiveSegmentedMax(tmpCartesianProductInsideProbabilities, tmpCartesianProductInsideProbabilities, tmpCartesianProductMidpoints,
		...
		scanner.scatter(tmpCartesianProductInsideProbabilities, cartesianProductProbabilities, tmpCartesianProductChildren, segmentFlags);
		scanner.scatter(tmpCartesianProductMidpoints, cartesianProductMidpoints, tmpCartesianProductChildren, segmentFlags);
