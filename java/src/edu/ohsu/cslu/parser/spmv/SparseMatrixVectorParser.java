 ...
 */
public abstract class SparseMatrixVectorParser<G extends SparseMatrixGrammar, C extends ParallelArrayChart> extends SparseMatrixParser<G, C> {
	...
	/**
	 * True if we're collecting detailed counts of cell populations, cartesian-product sizes, etc. Set from {@link ParserOptions}, but duplicated here as a final variable, so that
	 * the JIT can eliminate potentially-expensive counting code when we don't need it.
	 * 
	...
	/**
	 * Multiplies the grammar matrix (stored sparsely) by the supplied cartesian product vector (stored densely), and populates this chart cell.
	 * 
	...
	public final static class CartesianProductVector {
		...
		public CartesianProductVector(final SparseMatrixGrammar grammar, final float[] probabilities, final short[] midpoints, final int[] populatedLeftChildren, final int size) {
		...
		public CartesianProductVector(final SparseMatrixGrammar grammar, final float[] probabilities, final short[] midpoints, final int size) {
