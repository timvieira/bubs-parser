...
/**
 * Parses using Sparse Matrix-Vector multiplication, but performs the SpMV at each midpoint instead of first unioning the cartesian products and performing the SpMV once per cell.
 * 
 * @author Aaron Dunlop
 * @since Apr 13, 2010
 * 
...
public class CsrSpmvPerMidpointParser extends CsrSpmvParser {
	...
	protected void visitCell(final short start, final short end) {
		...
		if (end - start > 1) {
			...
			for (short midpoint = (short) (start + 1); midpoint <= end - 1; midpoint++) {
				...
				maxUnion(chartCellChildren, chartCellProbabilities, chartCellMidpoints, tmpChildren, tmpProbabilities, tmpMidpoints);
	...
	private void maxUnion(final int[] targetChildren, final float[] targetProbabilities, final short[] targetMidpoints, final int[] children2, final float[] probabilities2,
