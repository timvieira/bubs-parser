...
public class ChartCell {
	...
	private void buildLeftRightEdgeLists() {
		...
		if (bestEdgesHaveChanged) {
			...
			for (int i=0; i<bestEdge.length; i++) {
				tmpEdge = bestEdge[i];
				if (tmpEdge != null) {
					if (grammar.isLeftChild(tmpEdge.p.parent)) bestLeftEdges.add(tmpEdge);
					if (grammar.isRightChild(tmpEdge.p.parent)) bestRightEdges.add(tmpEdge);
				}
			}
    ...
    public int getNumEdgesAdded() { return numEdgesAdded; }
    
    public int getNumEdgeEntries() {
    	int numEntries=0;
    	for (int i=0; i<bestEdge.length; i++) {
    		if (bestEdge[i] != null) numEntries++;
    	}
    	return numEntries;
    }
