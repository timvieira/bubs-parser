...
/**
 * Grammar loop exhaustive parser using a sparse-matrix grammar representation ( {@link CsrSparseMatrixGrammar}). Loops over the entire grammar for each midpoint, probing child
 * cells for non-terminals matching each grammar rule.
 * 
 * @author Aaron Dunlop
 * @since Jun 13, 2010
 * 
		...
		for (short midpoint = (short) (start + 1); midpoint <= end - 1; midpoint++) {
			...
			for (int parent = 0; parent < v; parent++) {
				...
				for (int i = grammar.csrBinaryRowIndices[parent]; i < grammar.csrBinaryRowIndices[parent + 1]; i++) {
					...
					final float jointProbability = leftInsideProbability + rightInsideProbability + grammar.csrBinaryProbabilities[i];
