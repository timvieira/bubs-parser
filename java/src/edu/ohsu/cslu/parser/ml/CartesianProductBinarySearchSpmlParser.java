...
/**
 * Parser implementation which loops over all combinations of left and right child cell populations (cartesian product of observed left and right non-terminals) and probes into the
 * grammar for each combination using a binary search.
 * 
 * @author Aaron Dunlop
 * @since Jun 14, 2010
 * 
 ...
 */
public class CartesianProductBinarySearchSpmlParser extends SparseMatrixLoopParser<LeftCscSparseMatrixGrammar, PackedArrayChart> {
		...
		for (short midpoint = (short) (start + 1); midpoint <= end - 1; midpoint++) {
			...
			for (int i = leftStart; i <= leftEnd; i++) {
				...
				for (int j = rightStart; j <= rightEnd; j++) {
					...
					final int index = Arrays.binarySearch(grammar.cscBinaryPopulatedColumns, binarySearchStart, binarySearchEnd, childPair);
					if (index < 0) {
						continue;
					}
